   ...
   public function create(Request $request)
{
    ...
    $exchange_rate = !empty($business->exchange_rate) ? (float)$business->exchange_rate : 4000;
    ...
    // Variables for validation constraints
    $prefillData = null;
    $allowedCustomerRings = null; // Default: null means NO restrictions

    // Check if we are coming from a Reward/Sell transaction
    if ($request->has('sales_reward_id')) {
        $salesRewardId = $request->query('sales_reward_id');
        
        // Find the transaction (Sell or Reward Exchange)
        ...
        if ($rewardTransaction) {
            ...
            // 2. Auto get Invoice No
            if ($rewardTransaction->type == 'sell') {
                $invoice_sell_no = $rewardTransaction->invoice_no;
            } else {
                $invoice_sell_no = $rewardTransaction->ref_sale_invoice;
            }

            // 3. Calculate Allowed Quantities (Remaining Balance)
            $allowedCustomerRings = []; 
            $prefillProducts = [];
            $totalEntitlements = [];
            $usedQuantities = [];

            // A. Calculate Total Entitlements AND Subtract Used Ring Balance
            $sellLines = $rewardTransaction->sell_lines;
            ...
            foreach($rewards as $reward) {
                // Find the specific sell line for this reward's source product
                $sellLine = $sellLines->firstWhere('product_id', $reward->product_for_sale);
                if ($sellLine) {
                    $qty = $sellLine->quantity * $reward->exchange_quantity;
                    $exchangeProdId = $reward->exchange_product;
                    
                    if (isset($totalEntitlements[$exchangeProdId])) {
                        $totalEntitlements[$exchangeProdId] += $qty;
                    } else {
                        $totalEntitlements[$exchangeProdId] = $qty;
                    }

                    // --- CRITICAL FIX: Subtract Used Ring Balance ---
                    // This accounts for rings used/deducted at the time of sale (e.g., 23 used out of 46)
                    $usedInTransaction = $sellLine->used_ring_balance ?? 0;
                    if ($usedInTransaction > 0) {
                        if (isset($usedQuantities[$exchangeProdId])) {
                            $usedQuantities[$exchangeProdId] += $usedInTransaction;
                        } else {
                            $usedQuantities[$exchangeProdId] = $usedInTransaction;
                        }
                    }
            ...
            // B. Calculate Already Used Quantities from Previous Top Ups
            $existingTopUps = TransactionRingBalance::where('sell_ref_invoice', $invoice_sell_no)
                ->where('business_id', $business_id)
                ->where('type', 'top_up_ring_balance')
                ->whereNull('deleted_at')
            ...
            foreach($existingTopUps as $topUp) {
                foreach($topUp->transactionSellRingBalances as $line) {
                    // Only count customer rings (where cash_ring is null)
                    if (empty($line->cash_ring)) {
                        if (isset($usedQuantities[$line->product_id])) {
                            $usedQuantities[$line->product_id] += $line->quantity;
                        } else {
                            $usedQuantities[$line->product_id] = $line->quantity;
                        }
                    }
            ...
            // C. Calculate Final Remaining Allowed Limit
            foreach($totalEntitlements as $prodId => $total) {
                $used = $usedQuantities[$prodId] ?? 0;
                $remaining = max(0, $total - $used); // Ensure no negative values
                
                $allowedCustomerRings[$prodId] = $remaining;
            }

            // 4. Prepare Prefill Data for the View
            foreach($allowedCustomerRings as $prodId => $limit) {
                // Only prefill if limits exist
                ...
                if ($prod) {
                    $prefillProducts[] = [
                        ...
                        'type' => 'customer',
                        'cashRingValues' => null,
                        'max_allowed' => $limit // Pass the calculated remaining limit to the view
                    ];
                }
            }
    ...
    if ($contact_id && !Contact::where('id', $contact_id)->where('business_id', $business_id)->exists()) {
        abort(404, __('Contact not found.'));
    }
    ...
    $contacts = $customerContacts->mapWithKeys(function ($item) {
        return [$item->id => $item->name . ' (' . $item->contact_id . ')<br>Mobile: ' . ($item->mobile ?? '')];
    })->prepend(__('lang_v1.all'), '');
    ...
    // Pass $allowedCustomerRings to the view for validation
    return view('customer_ring_balance.create', compact('business_locations', 'contacts', 'suppliers', 'contact_id', 'prefillData', 'allowedCustomerRings', 'exchange_rate'));
}
    ...
    public function store(Request $request)
    {
        ...
        $transactionRingBalance = new TransactionRingBalance([
            ...
            'exchange_rate' => $data['exchange_rate'] ?? 4000, // Exchange rate field
            ...
            foreach ($data['products'] as $uniqueId => $productDetails) {
                ...
                if (isset($productDetails['cash_quantities']) && $productType === 'cash') {
                    $totalAmountInDollar = 0; // Store everything as USD for customer balance
                     ...
                     // Get exchange rate from form data, fallback to 4000 if empty/null/0
                    $exchangeRate = (!empty($data['exchange_rate']) && $data['exchange_rate'] > 0) ? $data['exchange_rate'] : 4000;
