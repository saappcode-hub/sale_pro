   ...
   public function index(Request $request)
{
    if ($request->ajax()) {
        ...
        if ($location_id && $location_id != 'all') {
            $baseQuery->where('location_id', $location_id);
        }

        if ($contact_id && $contact_id != 'all') {
            $baseQuery->where('contact_id', $contact_id);
        }
        ...
        if ($request->has('start_date') && $request->has('end_date') && 
            ...
            $baseQuery->whereBetween('transaction_date', [$start_date, $end_date]);
        }

        // 2. Apply search filter if exists
        $searchValue = $request->input('search.value');
        if (!empty($searchValue)) {
            // Find matching sales_order invoice numbers
            $matchingSalesOrderIds = Transaction::where('business_id', $business_id)
                ->where('type', 'sales_order')
                ->where('invoice_no', 'like', "%{$searchValue}%")
                ->pluck('id')
                ->toArray();
            
            // Find sell invoices that reference these sales orders
            $matchingSellInvoices = [];
            if (!empty($matchingSalesOrderIds)) {
                $matchingSellInvoices = Transaction::where('business_id', $business_id)
                    ->where('type', 'sell')
                    ->where(function($q) use ($matchingSalesOrderIds) {
                        foreach ($matchingSalesOrderIds as $soId) {
                            $q->orWhere('sales_order_ids', 'like', "%{$soId}%");
                        }
                    })
                    ->pluck('invoice_no')
                    ->toArray();
            ...
            $baseQuery->where(function($query) use ($searchValue, $matchingSellInvoices) {
                $query->where('ref_sale_invoice', 'like', "%{$searchValue}%")
                      ->orWhere('transaction_date', 'like', "%{$searchValue}%")
                      ->orWhereHas('contact', function($q) use ($searchValue) {
                ...
                if (!empty($matchingSellInvoices)) {
                    $query->orWhereIn('ref_sale_invoice', $matchingSellInvoices);
                }
            });
        }

        // 3. Get TOTAL transaction count (with search applied)
        $totalTransactions = (clone $baseQuery)->count();

        // 4. Manual Pagination
        $start = $request->input('start', 0);
        $length = $request->input('length', 25);
        if ($length == -1) { $length = $totalTransactions; }

        $transactions = $baseQuery
            ->select(['id', 'transaction_date', 'ref_sale_invoice', 'contact_id', 'location_id', 'status', 'business_id'])
        ...
        // 5. Expand Transactions into Product Rows
        $result = [];
        
        foreach ($transactions as $transaction) {
            // Get the sales_order invoice_no
            $salesOrderInvoiceNo = null;
            
            $sellTransaction = Transaction::where('invoice_no', $transaction->ref_sale_invoice)
                ->where('business_id', $business_id)
                ->where('type', 'sell')
                ->first();
            
            if ($sellTransaction && $sellTransaction->sales_order_ids) {
                $salesOrderIds = $sellTransaction->sales_order_ids;
                
                if (is_array($salesOrderIds)) {
                    $salesOrderId = !empty($salesOrderIds) ? $salesOrderIds[0] : null;
                } else {
                    $salesOrderIdsArray = json_decode($salesOrderIds, true);
                    if (is_array($salesOrderIdsArray) && !empty($salesOrderIdsArray)) {
                        $salesOrderId = $salesOrderIdsArray[0];
                    } else {
                        preg_match('/\d+/', $salesOrderIds, $matches);
                        $salesOrderId = $matches[0] ?? null;
                    }
                }
                
                if ($salesOrderId && is_string($salesOrderId)) {
                    preg_match('/\d+/', $salesOrderId, $matches);
                    $salesOrderId = $matches[0] ?? $salesOrderId;
                }
                
                if ($salesOrderId) {
                    $salesOrderTransaction = Transaction::where('id', $salesOrderId)
                        ->where('business_id', $business_id)
                        ->where('type', 'sales_order')
                        ->first();
                    
                    if ($salesOrderTransaction) {
                        $salesOrderInvoiceNo = $salesOrderTransaction->invoice_no;
                    }
                }
            }
            ...
            if ($rewards->isEmpty()) {
                $row = [
                    ...
                    'ref_sale_invoice' => $transaction->ref_sale_invoice,
                    'sales_order_invoice_no' => $salesOrderInvoiceNo ?? '-',
                ...
                foreach ($rewards as $index => $reward) {
                    ...
                    $row = [
                        ...
                        'contact' => $transaction->contact,
                        'location' => $transaction->location,
                        'exchange_product' => $reward->exchange_product ?? '-',
                        'receive_product' => $reward->receive_product ?? '-',
                        'set_quantity' => $quantity ? number_format($quantity, 2) : '0.00',
                        'used_ring_balance' => $usedRingBalance ? number_format($usedRingBalance, 2) : '0.00',
                        'transaction_id' => $transaction->id,
                        'is_first_row' => ($index === 0),
                        'product_count' => $rewards->count()
                    ];
                    $result[] = (object) $row;
                }
            }
        ...
        // 6. Return DataTables response
        return Datatables::of(collect($result))
            ->addColumn('action', function ($row) {
                $ringTopUpUrl = route('customer-ring-balance.create', ['sales_reward_id' => $row->id]);
                $ringTopUpButton = '<li><a href="' . $ringTopUpUrl . '"><i class="fa fa-arrow-circle-up" aria-hidden="true"></i> '.__('Ring Top up').'</a></li>';
                $editButton = '<li><a href="' . route('sales_reward.edit', $row->id) . '"><i class="fas fa-edit" aria-hidden="true"></i> '.__('messages.edit').'</a></li>';
                $view = '<li><a href="#" data-href="'.action([\App\Http\Controllers\SalesOrderRewardController::class, 'show'], [$row->id]).'" class="btn-modal" data-container=".view_modal"><i class="fas fa-eye" aria-hidden="true"></i> '.__('messages.view').'</a></li>';
            ...
            ->editColumn('reward_no', function($row) {
                return $row->ref_sale_invoice;
            })
            ->editColumn('sales_order_no', function($row) {
                return $row->sales_order_invoice_no;
            })
            ->editColumn('contact_name', function($row) {
                return $row->contact ? $row->contact->name : '';
            })
            ->editColumn('contact_mobile', function($row) {
            ...
            ->setTotalRecords($totalTransactions)
            ->setFilteredRecords($totalTransactions)
            ->skipPaging()
            ->rawColumns(['action'])
            ->make(true);
    }
