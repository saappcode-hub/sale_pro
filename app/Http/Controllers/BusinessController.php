...
class BusinessController extends Controller
{
    ...
    */
    ...
    public function postBusinessSettings(Request $request)
    {
        ...
        try {
            ...
            $checkboxes = ['enable_editing_product_from_purchase',
                'enable_inline_tax',
                'enable_brand', 'enable_category', 'enable_sub_category', 'enable_price_tax', 'enable_purchase_status','enable_purchase_type',
            ...
            $business->distant_for_salevisit = $request->input('distant_for_salevisit', 0);
    ...
    {
        if (!auth()->user()->can('business_settings.access')) abort(403, 'Unauthorized');
        
        $business_id = request()->session()->get('user.business_id');
        
        $types = \App\PurchaseType::where('business_id', $business_id)->get();
        
        // 1. Collect all Product IDs from all Purchase Types
        $all_product_ids = [];
        foreach ($types as $type) {
            // Check if exchange_product_id has data and is an array
            if (!empty($type->exchange_product_id) && is_array($type->exchange_product_id)) {
                $all_product_ids = array_merge($all_product_ids, $type->exchange_product_id);
            }
        }
        
        // 2. Fetch Product Names for these IDs
        $products = \App\Product::whereIn('id', array_unique($all_product_ids))
                        ->select('id', 'name', 'sku')
                        ->get()
                        ->keyBy('id');

        // 3. Format data for the frontend
        $formatted_types = $types->map(function($type) use ($products) {
            $product_names = [];
            $product_ids = [];

            if (!empty($type->exchange_product_id) && is_array($type->exchange_product_id)) {
                foreach ($type->exchange_product_id as $prod_id) {
            ...
            return [
                ...
                'exchange_product_id' => $product_ids, // The array of IDs
                'product_details' => $product_names,   // The array of Names for display
            ];
        });
    ...
    public function getExchangeProducts(Request $request)
    {
        $business_id = $request->session()->get('user.business_id');
        $term = $request->input('term');

        $query = \App\Product::join('rewards_exchange', 'products.id', '=', 'rewards_exchange.exchange_product')
                    ...
                    ->select(
                        ...
                        'products.name',
                        'products.sku as sub_sku' // CHANGED: Use 'sku' and alias it as 'sub_sku' for JS
                    )
                    ->groupBy('products.id');
        ...
        if (!empty($term)) {
            $query->where(function($q) use ($term) {
                $q->where('products.name', 'like', '%' . $term . '%')
                  ->orWhere('products.sku', 'like', '%' . $term . '%'); // CHANGED: products.sku instead of products.sub_sku
            });
        }
    ...
    public function savePurchaseType(Request $request)
    {
        if (!auth()->user()->can('business_settings.access')) abort(403, 'Unauthorized');
        ...
        try {
            ...
            // Save the array directly into 'exchange_product_id'
            // The model cast will handle JSON conversion
            $type = \App\PurchaseType::create([
                'name' => $request->input('name'),
                'business_id' => request()->session()->get('user.business_id'),
                'exchange_product_id' => $request->input('exchange_products') // From multiple select
            ]);
            
            return response()->json(['success' => true, 'msg' => __('lang_v1.added_success'), 'data' => $type]);
        } catch (\Exception $e) {
            \Log::emergency("File:" . $e->getFile(). "Line:" . $e->getLine(). "Message:" . $e->getMessage());
    ...
    public function updatePurchaseType(Request $request)
    {
        ...
        try {
            ...
            if (empty($request->name)) {
                return response()->json(['success' => false, 'msg' => 'Name is required']);
            }

            $type = \App\PurchaseType::where('business_id', $business_id)
                        ->where('id', $request->id)
                        ->first();
            
            if($type){
                $type->name = $request->name;
                $type->exchange_product_id = $request->input('exchange_products'); // Update Array
                $type->save(); 
            }
    ...
    public function deletePurchaseType($id)
    {
        ...
        try {
            ...
            $type = \App\PurchaseType::where('business_id', $business_id)->where('id', $id)->first();
            
            if ($type) {
                $type->delete(); // This will set 'deleted_at' timestamp
                return response()->json(['success' => true, 'msg' => __('lang_v1.deleted_success')]);
            } else {
                return response()->json(['success' => false, 'msg' => 'Type not found']);
            }
