...
class AdminSidebarMenu
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        Menu::create('admin-sidebar-menu', function ($menu) {
            ...
            $business_owner_id = session('business.owner_id');
            $is_business_owner = ($current_user_id == $business_owner_id);
                ...
                $menu->dropdown(
                    ...
                    function ($sub) {
                        if (auth()->user()->can('product.view')) {
                            ...
                            // $sub->url(
                            //     action([\App\Http\Controllers\ProductController::class, 'index_sync']),
                            //     __('Sync Product'),
                            //     ['icon' => 'fa fas fa-circle', 'active' => request()->segment(1) == 'index_sync']
                            // );
                        }
            ...
            if ($is_admin || auth()->user()->hasAnyPermission(['sell.view', 'sell.create', 'direct_sell.access', 'view_own_sell_only', 'view_commission_agent_sell', 'access_shipping', 'access_own_shipping', 'access_commission_agent_shipping', 'access_sell_return', 'direct_sell.view', 'direct_sell.update', 'access_own_sell_return'])) {
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules, $is_admin, $pos_settings) {
                        ...
                        $sub->url(
                            action([\App\Http\Controllers\CustomerContractController::class, 'index']),
                            'List Contract',
                            ['icon' => 'fa fas fa-file-contract', 'active' => request()->segment(1) == 'customer-contracts-list']
                        );
                        $sub->url(
                            action([\App\Http\Controllers\SellController::class, 'avoidSellIndex']),
                            __('Void Sell'),
                            ['icon' => 'fa fa-ban', 'active' => request()->segment(2) == 'avoid']
                        );
            ...
            if (in_array('rewards_exchange', $enabled_modules)) {
                $menu->dropdown(
                    __('Reward Exchange'),
                    function ($sub) {  
                        $sub->url(
                            action([\App\Http\Controllers\SalesOrderRewardController::class, 'index']),
                            __('Customer Rewards Exchange'),
                            ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'sales_reward']
                        );
                        $sub->url(
                            action([\App\Http\Controllers\CustomerRingBalanceController::class, 'index']),
                            __('Customer Ring Top Up'),
                            ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'customer-ring-balance']
                        );
                        // $sub->url(
                        //     action([\App\Http\Controllers\AllRingBalanceController::class, 'index']),
                        //     __('All Ring'),
                        //     ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'all-ring']
                        // );
                        $sub->url(
                            action([\App\Http\Controllers\CustomerRingController::class, 'index']),
                            __('Customer Ring Balance'),
                            ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'customer-ring']
                        );
                        $sub->url(
                            action([\App\Http\Controllers\SupplierCashRingBalanceController::class, 'index']),
                            __('Supplier Exchange(Ring Cash)'),
                            ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'supplier-cash-ring-balance']
                        );
                        $sub->url(
                            action([\App\Http\Controllers\SalesOrderRewardSupllierController::class, 'index']),
                            __('Supplier Exchange(Can)'),
                            ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'sale-reward-supplier']
                        );
                        $sub->url(
                            action([\App\Http\Controllers\SalesOrderRewardSupllierReceiveController::class, 'index']),
                            __('Supplier Rewards Stock Receive'),
                            ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'sale-reward-supplier-receive']
                        );
                    },
                    ['icon' => 'fa fas fa-arrow-circle-up', 'id' => 'tour_step7']
                )->order(35);
            ...
            //Purchase dropdown
            if (in_array('warehouse', $enabled_modules) ) {
                $menu->dropdown(
                    __('Warehouse'),
                    function ($sub) { 
                        $sub->url(
                            action([\App\Http\Controllers\PurchaseStockReceiveController::class, 'index']),
                            __('Purchase Order'),
                            ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'purchase-stock-receive']
                        );
                        $sub->url(
                            action([\App\Http\Controllers\WarehouseController::class, 'index']),
                            __('Stock Receive'),
                            ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'warehouse' && request()->segment(2) == null]
                        );
                        $sub->url(
                            action([\App\Http\Controllers\SalesOrderScanController::class, 'index']),
                            __('Warehouse Scan'),
                            ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'sale-order-scan' && request()->segment(2) == null]
                        );
                    },
                    ['icon' => 'fa fa-university', 'id' => 'tour_step7']
                )->order(35);
            }
            ...
            if (auth()->user()->can('account.access') && in_array('account', $enabled_modules)) {
                $menu->dropdown(
                    ...
                    function ($sub) {
                        ...
                        $sub->url(
                            action([\App\Http\Controllers\CashDepositController::class, 'index']),
                            'Cash Deposit', 
                            ['icon' => 'fa fas fa-money-bill', 'active' => request()->segment(1) == 'cash-deposit']
                        );
                        $sub->url(
            ...
            if (auth()->user()->can('purchase_n_sell_report.view') || auth()->user()->can('contacts_report.view')
                ...
                || auth()->user()->can('register_report.view') || auth()->user()->can('expense_report.view') || auth()->user()->can('ring_report.view') 
                // Add new permissions to the main check
                || auth()->user()->can('stock_movement_report.view') || auth()->user()->can('daily_sales_payment_report.view')
                || auth()->user()->can('daily_payment_report.view') || auth()->user()->can('sales_report_summary.view')
                || auth()->user()->can('sales_detail_report.view') || auth()->user()->can('sales_details_sellers_report.view')
                || auth()->user()->can('report_export_center.view') || auth()->user()->can('sales_order_detail_report.view')
                ) {
                $menu->dropdown(
                __('report.reports'),
                function ($sub) use ($enabled_modules, $is_admin, $is_business_owner) {
                    ...
                    $sub->url(
                        action([\App\Http\Controllers\ReportController::class, 'SaleRevenueAR']),
                        __('Sale Revenue & A/R'),
                        ['icon' => 'fa fas fa-chart-line', 'active' => request()->segment(2) == 'sale-revenue-ar']
                    );
                    ...
                    if (session('business.id') == 53) {
                        $sub->url(
                            action([\App\Http\Controllers\ReportController::class, 'ClaimReport']),
                            __('Claim Report'),
                            ['icon' => 'fa fas fa-file-invoice', 'active' => request()->segment(2) == 'claim-report']
                        );
                    }

                    // ADDED PERMISSION CHECK
                    if (auth()->user()->can('stock_movement_report.view')) {
                        $sub->url(
                            action([\App\Http\Controllers\StockMovementReportController::class, 'index']),
                            __('Stock Movement Report'),
                            ['icon' => 'fa fas fa-hourglass-half', 'active' => request()->segment(2) == 'stock-movement-report']
                        );
                    }
                    
                    // MOVED RING REPORT TO BE INDEPENDENT
                    if (auth()->user()->can('ring_report.view')) {
                        $sub->url(
                            action([\App\Http\Controllers\ReportController::class, 'DailyRingTopUp']),
                            __('Ring Report'),
                            ['icon' => 'fa fas fa-calendar-day', 'active' => request()->segment(2) == 'daily-ring-topup']
                        );
                    }
                    
                    // ADDED PERMISSION CHECK
                    if (auth()->user()->can('daily_sales_payment_report.view')) {
                        $sub->url(
                            action([\App\Http\Controllers\ReportController::class, 'DailySalesPaymentReport']),
                            __('Daily Sales & Payment Report'),
                            ['icon' => 'fa fas fa-calendar-day', 'active' => request()->segment(2) == 'daily-sales-payment-report']
                        );
                    }

                    // ADDED PERMISSION CHECK
                    if (auth()->user()->can('daily_payment_report.view')) {
                        $sub->url(
                            action([\App\Http\Controllers\ReportController::class, 'DailyPaymentReport']),
                            __('Daily Payment Report'),
                            ['icon' => 'fa fas fa-calendar-day', 'active' => request()->segment(2) == 'daily-payment-report']
                        );
                    }
                    
                    // ADDED PERMISSION CHECK for the outer condition
                    if (auth()->user()->can('sales_report_summary.view')) {
                        // Inner check for owner/admin remains
                        $sub->url(
                            action([\App\Http\Controllers\ReportController::class, 'SalesReportSummary']),
                            __('Sales Report Summary'),
                            ['icon' => 'fa fas fa-calendar-day', 'active' => request()->segment(2) == 'sales-report-summary']
                        );
                    }

                    // ADDED PERMISSION CHECK
                    if (auth()->user()->can('sales_detail_report.view')) {
                        $sub->url(
                            action([\App\Http\Controllers\ReportController::class, 'SaleDetailReport']),
                            __('Sales Detail Report'),
                            ['icon' => 'fa fas fa-calendar-day', 'active' => request()->segment(2) == 'sales-detail-report']
                        );
                    }
                    if (auth()->user()->can('sales_order_detail_report.view')) {
                        $sub->url(
                            action([\App\Http\Controllers\ReportController::class, 'SalesOrderDetailReport']),
                            __('Sales Order Detail Report'),
                            ['icon' => 'fa fas fa-calendar-day', 'active' => request()->segment(2) == 'sales-order-detail-report']
                        );
                    }
                    // ADDED PERMISSION CHECK
                    if (auth()->user()->can('sales_details_sellers_report.view')) {
                        $sub->url(
                            action([\App\Http\Controllers\ReportController::class, 'SaleDetailReportForSell']),
                            __('Sales Details Report (Sellers)'),
                            ['icon' => 'fa fas fa-calendar-day', 'active' => request()->segment(2) == 'sales-detail-report-sale']
                        );
                    }
                    
                    // ADDED PERMISSION CHECK
                    if (auth()->user()->can('report_export_center.view')) {
                        $sub->url(
                            action([\App\Http\Controllers\ReportController::class, 'ReportExportCenter']),
                            __('Report Export Center'),
                            ['icon' => 'fa fas fa-file-export', 'active' => request()->segment(2) == 'report-export-center']
                        );
                    }
            ...
            if (auth()->user()->can('business_settings.access') ||
                auth()->user()->can('currencies_settings.access') ||
                    ...
                    function ($sub) use ($enabled_modules) {
                        ...
                        if(auth()->user()->can('currencies_settings.access')){
                            $sub->url(
                                ...
                                __('Currecies Settings'),
                                ['icon' => 'fa fas fa-map-marker', 'active' => request()->segment(1) == 'currencies-settings']
                            );
                        }
                        if (auth()->user()->can('invoice_settings.access')) {
                            $sub->url(
                                ...
                                ['icon' => 'fa fas fa-file', 'active' => in_array(request()->segment(1), ['invoice-schemes', 'invoice-layouts', 'invoice-qrs'])]
                            );
                        }
                        ...
                        if (in_array('rewards_exchange', $enabled_modules)) {
                            $sub->url(
                                action([\App\Http\Controllers\RewardExchangeController::class, 'index']),
                                __('Rewards'),
                                ['icon' => 'fa fas fa-user-circle', 'active' => request()->segment(1) == 'rewards_exchange']
                            );
                        }
                        $sub->url(
                            action([\App\Http\Controllers\TelegramSettingController::class, 'index']),
                            __('Telegram Setting'),
                            [
                                'icon'   => 'fa fa-telegram',
                                'active' => request()->segment(1) == 'telegram-setting',
                            ]
                        );
                        $sub->url(
            ...
            $menu->dropdown(
                ...
                function ($sub) {
                        $sub->url(
                            action([\App\Http\Controllers\SalesOrderVisitController::class, 'index']),
                            __('List Sale Visit'),
                            ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'sales_visit']
                        );
                        // if (session('business.id') == 53) {
                            $sub->url(
                                action([\App\Http\Controllers\SalesOrderVisitController::class, 'dailySummary']),
                                __('Daily Sale Visit Summary'),
                                ['icon' => 'fa fas fa-chart-pie', 'active' => request()->segment(1) == 'daily-sale-visit-summary']
                            );
                        // }
                        $sub->url(
                            action([\App\Http\Controllers\ReportSaleTrackingController::class, 'index']),
                            __('Report Sale Tracking'),
                            ['icon' => 'fa fas fa-user-circle', 'active' => request()->segment(1) == 'sale-tracking-report']
                        );
                        $sub->url(
                            action([\App\Http\Controllers\GpsTrackingController::class, 'index']),
                            __('GPS Tracking'),
                            ['icon' => 'fa fas fa-user-circle', 'active' => request()->segment(1) == 'gps-tracking']
                        );
                },
                ['icon' => 'fa fas fa-arrow-circle-up', 'id' => 'tour_step7']
            )->order(30);
