...
class BusinessUtil extends Util
{
    ...
    public function newBusinessDefaultResources($business_id, $user_id)
    {
        ...
        //create default invoice layour for new business
        InvoiceQrs::create(['name' => 'Default',
            'header_text' => null,
            'invoice_no_prefix' => 'Invoice No.',
            'invoice_heading' => 'Invoice',
            'sub_total_label' => 'Subtotal',
    ...
    public function createNewBusiness($business_details)
    {
        ...
        $business_details['ref_no_prefixes'] = [
            ...
            'ring_topup' => 'RINV', // NEW: Added Ring Top Up prefix
        ];
    ...
    public function createDefaultSellingPriceGroup($businessId)
    {
        SellingPriceGroup::create([
            'name' => 'Normal',
    ...
    /**
     * Creates default shipping labels (Work and Home) for a business.
     *
     * @param int $businessId
     * @return void
     */
    public function createDefaultShippingLabels($businessId)
    {
        $labels = [
            ['name' => 'Home', 'business_id' => $businessId],
            ['name' => 'Work', 'business_id' => $businessId],
        ];

        foreach ($labels as $label) {
            LabelShipping::create($label);
        }
    }

    public function createDefaultCustomerGroupWithDefaultSellingGroup($businessId)
    {
        // Fetch the default selling price group for the business
        $defaultSellingPriceGroup = SellingPriceGroup::where('business_id', $businessId)
            ->where('is_default', 1)
            ->first();

        // Fetch the first user associated with this business (owner or any user)
        $user = User::where('business_id', $businessId)->first();

        // If no user is found, set a default admin ID (adjust as needed)
        $userID = $user ? $user->id : 1;

        // Create the default customer group with a linked selling price group
        CustomerGroup::create([
            'business_id' => $businessId,
            'name' => 'Normal',
            'amount' => 0, // Default amount (adjust if needed)
            'price_calculation_type' => 'selling_price_group', // Default calculation type
            'selling_price_group_id' => $defaultSellingPriceGroup ? $defaultSellingPriceGroup->id : null,
            'is_default' => 1,
            'created_by' => $userID // Ensure a valid user ID
        ]);
    }
    ...
    {
        // ✅ FIX 1: Add validation for business_id
        if (!$business_id) {
            return [
                'start' => date('Y-01-01'),
                'end' => date('Y-12-31'),
        ...
        // ✅ FIX 2: Get business and check if it exists
        $business = Business::where('id', $business_id)->first();
        
        if (!$business) {
            \Log::warning("Business not found with ID: {$business_id}");
            // Return default financial year (January - December)
            return [
                'start' => date('Y-01-01'),
                'end' => date('Y-12-31'),
            ];
        ...
        // ✅ FIX 3: Check if fy_start_month exists and has a valid value
        $start_month = $business->fy_start_month ?? 1; // Default to January if not set
        
        if (!is_numeric($start_month) || $start_month < 1 || $start_month > 12) {
            $start_month = 1; // Default to January if invalid
        }
     ...
     */
    public function addLocation($business_id, $location_details, $invoice_scheme_id = null, $invoice_layout_id = null, $invoice_qrs_id = null)
    {
        ...
        if (empty($invoice_scheme_id)) {
            $scheme = InvoiceQrs::where('is_default', 1)
                                    ->where('business_id', $business_id)
                                    ->first();
            $invoice_qrs_id = $scheme->id;
        }
        ...
        $location = BusinessLocation::create(['business_id' => $business_id,
            ...
            'invoice_qrs_id ' => $invoice_qrs_id,
            'sale_invoice_qrs_id ' => $invoice_qrs_id,
