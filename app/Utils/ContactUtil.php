...
class ContactUtil extends Util
{
    ...
    public function getContactQuery($business_id, $type, $contact_ids = [])
    {
        $query = Contact::leftjoin('transactions AS t', 'contacts.id', '=', 't.contact_id')
                    ...
                    ->leftjoin('contacts_map AS cm', 'contacts.id', '=', 'cm.contact_id') // Join contacts_map
                    ->leftjoin('cambodia_provinces AS p', 'cm.province_id', '=', 'p.id') // Join provinces
                    ->leftjoin('cambodia_districts AS d', 'cm.district_id', '=', 'd.id') // Join districts
                    ->leftjoin('cambodia_communes AS c', 'cm.commune_id', '=', 'c.id') // Join communes
                    ->where('contacts.business_id', $business_id);
        if ($type == 'supplier') {
            $query->onlySuppliers();
        ...
        $query->select([
            ...
            't.transaction_date',
            DB::raw("CONCAT(IFNULL(p.name_en, ''), ' ', IFNULL(d.name_en, ''), ' ', IFNULL(c.name_en, '')) AS location") // Replace commas with spaces
        ]);
