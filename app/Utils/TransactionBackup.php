...
class TransactionBackup extends Util
{
    /**
        ...
        if (! empty($lines_formatted)) {
            ...
            foreach ($lines_formatted as $key => $value) {
                ...
                if (! empty($products_modified_combo[$key]['product_type']) && $products_modified_combo[$key]['product_type'] == 'combo') {
                    $combo_lines = array_merge($combo_lines, $this->__makeLinesForComboProduct($products_modified_combo[$key]['combo'], $value, 'combo', $products_modified_combo[$key]['quantity']));
                }
                if (! empty($products_modified_combo[$key]['product_type']) && $products_modified_combo[$key]['product_type'] == 'combo_single') {
                    $combo_lines = array_merge($combo_lines, $this->__makeLinesForComboProduct($products_modified_combo[$key]['combo_single'], $value, 'combo_single', $products_modified_combo[$key]['quantity']));
                }
    ...
    public function saveCurrentStockBackup($transaction, $products, $business_id, $location_id, $uf_data = true, $t_type = 'create_sell')
    {
        ...
        foreach ($products as $product) {
            ...
            \Log::info("Processing product in saveCurrentStockBackup: " . json_encode($product));
            ...
            if (!empty($product['sub_unit_id']) && isset($product['base_unit_multiplier']) && $product['base_unit_multiplier'] > 0) {
                $multiplier = $product['base_unit_multiplier'];
                \Log::info("Using multiplier: $multiplier (sub_unit_id: {$product['sub_unit_id']})");
            } else {
                $multiplier = 1; // Default to 1 if sub_unit_id is null or base_unit_multiplier is not set/valid
                \Log::info("Defaulting multiplier to 1 (sub_unit_id: " . (isset($product['sub_unit_id']) ? $product['sub_unit_id'] : 'null') . ")");
            }
            ...
            \Log::info("Calculated quantity: $quantity (uf_quantity: $uf_quantity, multiplier: $multiplier)");
    
            if ($t_type == 'create_sell') {
                // For create_sell, get the last qty_change for the product_id and location_id (across all variations)
                ...
                $current_stock = $latest_stock ? $latest_stock->qty_change : 0;
                \Log::info("Last qty_change for product_id {$product['product_id']} at location_id {$location_id}: $current_stock");
    
                // For create_sell: Subtract the quantity from the last stock
                $qty_change = $current_stock - $quantity;
                ...
                if ($last_create_sell) {
                    $quantity = $last_create_sell->qty; // Use the original quantity from the last create_sell (already in base unit)
                    \Log::info("Found last create_sell for transaction_id {$transaction->id}, qty: $quantity (reversing this quantity)");
                } else {
                    // If no previous create_sell is found, log an error and skip this product
                    \Log::error("No previous create_sell found for transaction_id {$transaction->id}, product_id {$product['product_id']}, variation_id {$product['variation_id']}, location_id {$location_id}");
                    continue;
                }
                ...
                // Calculate new qty_change for edit_sell_reversal
                $qty_change = $latest_stock ? $latest_stock->qty_change + $quantity : $quantity;
                \Log::info("Reversal (edit_sell_reversal): Adding back $quantity units, qty_change: $qty_change");
                ...
                \Log::info("Deleting product, using quantity: $quantity for product_id {$product['product_id']}, variation_id {$product['variation_id']}, location_id {$location_id}");
                ...
                // Calculate new qty_change by adding back the deleted quantity
                $qty_change = $latest_stock ? $latest_stock->qty_change + $quantity : $quantity;
                \Log::info("Delete product (delete_product_sell): Adding back $quantity units, qty_change: $qty_change");
            }
    
            // Log the calculated qty_change
            \Log::info("Calculated qty_change: $qty_change (t_type: $t_type)");
            ...
            $stock_line = [
                ...
                't_type' => $t_type,
                'qty' => $quantity, // Store as positive (in base unit)
                'qty_change' => $qty_change, // Stock level after the transaction
        ...
        if (!empty($stock_lines)) {
            ...
            \Log::warning("No stock lines to save in saveCurrentStockBackup");
        }
     ...
     private function __makeLinesForComboProduct($combo_items, $parent_sell_line, $combo, $qtyforSingle)
    {
        ...
        foreach ($combo_items as $value) {
            ...
            $sell_line = TransactionSellLine::create([
                ...
                'quantity' => $combo == 'combo_single' ? $value['quantity'] * $qtyforSingle : $value['quantity'],
    ...
    public function editSellLine($product, $location_id, $status_before, $multiplier = 1, $uf_data = true)
    {
        ...
        if (isset($product['product_type']) && $product['product_type'] == 'combo' && !empty($product['combo'])) {
            ...
            $this->updateEditedSellLineCombo($product['combo'], $location_id, $adjust_stock, $old_qty, $this->num_uf($product['quantity']), 'combo');
        }

        if (isset($product['product_type']) && $product['product_type'] == 'combo_single' && !empty($product['combo_single'])) {
            foreach ($product['combo_single'] as $combo_line) {
                $edit_ids[] = $combo_line['transaction_sell_lines_id'];
            }
            $adjust_stock = ($status_before != 'draft');
            $this->updateEditedSellLineCombo($product['combo_single'], $location_id, $adjust_stock, $old_qty, $this->num_uf($product['quantity']), 'combo_single');
        }
    ...
    public function deleteSale($business_id, $transaction_id)
    {
        ...
        if ($this->isReturnExist($transaction_id)) {
            \Log::warning("Cannot delete transaction_id $transaction_id: Return exists.");
            return [
                'success' => false,
        ...
        if (empty($transaction)) {
            \Log::error("Transaction with ID $transaction_id not found for business_id $business_id.");
            return [
                'success' => false,
        ...
        \Log::info("Found transaction with ID $transaction_id. Status: {$transaction->status}, Type: {$transaction->type}, Location: {$transaction->location_id}");
        ...
        if ($transaction->status == 'draft') {
            \Log::info("Transaction ID $transaction_id is in draft status. Deleting directly.");
            foreach ($transaction->sell_lines as $sell_line) {
            ...
            $transaction->delete();
            \Log::info("Draft transaction ID $transaction_id deleted successfully.");
        } else {
            ...
            \Log::info("Processing deletion for final transaction ID $transaction_id. Sell lines: " . count($deleted_sell_lines));

            // Before deleting, handle stock backup for delete_sell
            $stock_lines = [];
            foreach ($deleted_sell_lines as $sell_line) {
                // Log the sell line for debugging
                \Log::info("Processing sell line for deletion: " . json_encode($sell_line->toArray()));

                // Fetch the last qty_change for the product_id and location_id (including the current transaction)
                $latest_stock = CurrentStockBackup::where('business_id', $business_id)
                ...
                if ($latest_stock) {
                    \Log::info("Found latest stock for product_id {$sell_line->product_id} at location_id {$transaction->location_id}: " . json_encode($latest_stock->toArray()));
                } else {
                    \Log::warning("No stock records found for product_id {$sell_line->product_id} at location_id {$transaction->location_id}.");
                }

                $current_stock = $latest_stock ? $latest_stock->qty_change : 0;
                \Log::info("Last qty_change for product_id {$sell_line->product_id} at location_id {$transaction->location_id}: $current_stock");

                // Calculate the quantity to add back (already in base unit in TransactionSellLine)
                $quantity = $sell_line->quantity;
                \Log::info("Quantity to add back for product_id {$sell_line->product_id}: $quantity");

                // Calculate new qty_change (add back the quantity since we're reversing a sell)
                $qty_change = $current_stock + $quantity;
                \Log::info("Calculated qty_change for delete_sell: $qty_change (current_stock: $current_stock + quantity: $quantity)");

                // Prepare stock backup line for delete_sell with location_id
                $stock_line = [
                    'business_id' => $business_id,
            ...
            // Save the delete_sell stock backup lines
            if (!empty($stock_lines)) {
                \Log::info("Saving delete_sell stock lines: " . json_encode($stock_lines));
                CurrentStockBackup::insert(
                    array_map(function ($line) {
                        return $line->toArray();
                    }, $stock_lines)
                ...
                \Log::warning("No stock lines to save for delete_sell in transaction_id {$transaction_id}");
            }
            ...
            \Log::info("Marked transaction ID $transaction_id as draft before deletion.");
            ...
            \Log::info("Deleted cash register payments for transaction ID $transaction_id.");

            $transaction->delete();
            \Log::info("Final transaction ID $transaction_id deleted successfully.");
            ...
            if (array_key_exists($sell_line->id, $returns)) {
                ...
                if (!empty($sell_return_before)) {
                    ...
                    if ($previous_sell_return) {
                        ...
                        // Fetch the last qty_change before the previous create_sell_return
                        ...
                        $current_stock = $last_stock_before_previous ? $last_stock_before_previous->qty_change : 0;
                        \Log::info("Last qty_change before previous create_sell_return for product_id {$sell_line->product_id} at location_id {$sell_return->location_id}: $current_stock");
                        ...
                        $reversal_stock_line = [
                            ...
                            'qty' => $previous_sell_return->qty,
                            'qty_change' => $current_stock,
                            'transaction_date' => now(),
                        ];
                ...
                // If no reversal was needed, fetch the last qty_change from the database
                if (empty($sell_return_before) || !$previous_sell_return) {
                    ...
                    $current_stock = $latest_stock ? $latest_stock->qty_change : 0;
                    \Log::info("Last qty_change for product_id {$sell_line->product_id} at location_id {$sell_return->location_id}: $current_stock");
                }

                // Calculate new qty_change (add the returned quantity to increase stock)
                $qty_change = $current_stock + $quantity;
                \Log::info("Calculated qty_change for create_sell_return: $qty_change (current_stock: $current_stock + quantity: $quantity)");
