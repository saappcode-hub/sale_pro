...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        ...
        $transaction = Transaction::create([
            ...
            'currency_id' => ! empty($input['currency_id']) ? $input['currency_id'] : 124,
    ...
    public function createOrUpdateSellLines($transaction, $products, $location_id, $return_deleted = false, $status_before = null, $extra_line_parameters = [], $uf_data = true)
{
    ...
    // Track sales order line updates for combo products
    $so_line_updates = [];
    ...
    if (!empty($edit_ids)) {
        $deleted_lines = TransactionSellLine::where('transaction_id', $transaction->id)
            ...
            ->whereNotIn('id', function ($query) use ($edit_ids) {
                // Exclude child lines with children_type = 'combo_single' whose parent_sell_line_id is in edit_ids
                $query->select('id')
                    ->from('transaction_sell_lines')
                    ->where('children_type', 'combo_single')
                    ->whereIn('parent_sell_line_id', $edit_ids);
            })
            ->select('id')
            ->get()
            ->pluck('id')
        ...
        // Include both combo and combo_single child lines for deletion if their parent is deleted
        $combo_delete_lines = TransactionSellLine::whereIn('parent_sell_line_id', $deleted_lines)
            ->whereIn('children_type', ['combo', 'combo_single'])
            ->select('id')
            ->get()
            ->pluck('id')
            ->toArray();
    ...
    if (! empty($lines_formatted)) {
        ...
        foreach ($lines_formatted as $key => $value) {
            if (! empty($products_modified_combo[$key]['product_type']) && $products_modified_combo[$key]['product_type'] == 'combo') {
                $combo_lines_temp = $this->__makeLinesForComboProduct($products_modified_combo[$key]['combo'], $value, 'combo', $products_modified_combo[$key]['quantity']);
                $combo_lines = array_merge($combo_lines, $combo_lines_temp);
                
                // Track combo sub-products for sales order updates
                foreach ($combo_lines_temp as $combo_line) {
                    $so_line_updates[] = [
                        'variation_id' => $combo_line->variation_id,
                        'quantity' => $combo_line->quantity,
                        'main_so_line_id' => $value->so_line_id
                    ];
                }
            }
            if (! empty($products_modified_combo[$key]['product_type']) && $products_modified_combo[$key]['product_type'] == 'combo_single') {
                $combo_lines_temp = $this->__makeLinesForComboProduct($products_modified_combo[$key]['combo_single'], $value, 'combo_single', $products_modified_combo[$key]['quantity']);
                $combo_lines = array_merge($combo_lines, $combo_lines_temp);
                
                // Track combo_single sub-products for sales order updates
                foreach ($combo_lines_temp as $combo_line) {
                    $so_line_updates[] = [
                        'variation_id' => $combo_line->variation_id,
                        'quantity' => $combo_line->quantity,
    ...
    // IMPORTANT: Update so_quantity_invoiced for combo sub-products
    if (!empty($so_line_updates)) {
        \Log::info('=== Updating Sales Order Lines for Combo Products ===', [
            'so_line_updates' => $so_line_updates
        ]);
        
        foreach ($so_line_updates as $update) {
            if (!empty($update['main_so_line_id'])) {
                // Find the sales order transaction
                $main_so_line = TransactionSellLine::find($update['main_so_line_id']);
                if ($main_so_line) {
                    // Find the corresponding combo sub-product in the sales order
                    $so_combo_line = TransactionSellLine::where('transaction_id', $main_so_line->transaction_id)
                        ->where('parent_sell_line_id', $update['main_so_line_id'])
                        ->where('variation_id', $update['variation_id'])
                        ->first();
                    
                    if ($so_combo_line) {
                        \Log::info('Updating SO combo line', [
                            'so_combo_line_id' => $so_combo_line->id,
                            'variation_id' => $update['variation_id'],
                            'old_so_quantity_invoiced' => $so_combo_line->so_quantity_invoiced,
                            'adding_quantity' => $update['quantity']
                        ]);
                        
                        // Update the so_quantity_invoiced for the combo sub-product
                        $so_combo_line->so_quantity_invoiced += $update['quantity'];
                        $so_combo_line->save();
                        
                        \Log::info('Updated SO combo line', [
                            'new_so_quantity_invoiced' => $so_combo_line->so_quantity_invoiced
                        ]);
                    } else {
                        \Log::warning('Could not find SO combo line', [
                            'transaction_id' => $main_so_line->transaction_id,
                            'parent_sell_line_id' => $update['main_so_line_id'],
                            'variation_id' => $update['variation_id']
                        ]);
                    }
                }
    ...
    /**
     * Saves stock history to current_stock_backup for a sell transaction.
     *
     * @param \App\Transaction $transaction
     * @param array $products
     * @param int $business_id
     * @param int $location_id
     * @param bool $uf_data Whether to unformat data (default: true)
     * @return bool
     */
    public function saveCurrentStockBackup($transaction, $products, $business_id, $location_id, $uf_data = true, $t_type = 'create_sell')
    {
        $stock_lines = [];
    
        foreach ($products as $product) {
            $multiplier = 1;
            // Handle sub_unit_id and base_unit_multiplier properly
            if (!empty($product['sub_unit_id']) && isset($product['base_unit_multiplier']) && $product['base_unit_multiplier'] > 0) {
                $multiplier = $product['base_unit_multiplier'];
            } else {
                $multiplier = 1; // Default to 1 if sub_unit_id is null or base_unit_multiplier is not set/valid
            }
    
            // Calculate quantity (positive for sell)
            $uf_quantity = $uf_data ? $this->num_uf($product['quantity']) : $product['quantity'];
            $quantity = $uf_quantity * $multiplier; // Positive quantity
    
            if ($t_type == 'create_sell') {
                // For create_sell, get the last new_qty for the product_id and location_id (across all variations)
                $latest_stock = CurrentStockBackup::where('business_id', $business_id)
                    ->where('product_id', $product['product_id'])
                    ->where('location_id', $location_id)
                    ->orderBy('transaction_date', 'desc')
                    ->orderBy('id', 'desc') // Ensure we get the latest record in case of same timestamp
                    ->first();
    
                $current_stock = $latest_stock ? $latest_stock->new_qty : 0;
    
                // For create_sell: Subtract the quantity from the last stock
                $new_qty = $current_stock - $quantity;
            } elseif ($t_type == 'edit_sell_reversal') {
                // For edit_sell_reversal, fetch the last create_sell for this transaction_id, product_id, variation_id, and location_id
                $last_create_sell = CurrentStockBackup::where('business_id', $business_id)
                    ->where('product_id', $product['product_id'])
                    ->where('variation_id', $product['variation_id'])
                    ->where('location_id', $location_id)
                    ->where('transaction_id', $transaction->id)
                    ->where('t_type', 'create_sell')
                    ->orderBy('transaction_date', 'desc')
                    ->orderBy('id', 'desc') // Ensure we get the latest create_sell
                    ->first();
    
                if ($last_create_sell) {
                    $quantity = $last_create_sell->qty_change; // Use the original quantity from the last create_sell (already in base unit)
                } else {
                    // If no previous create_sell is found, log an error and skip this product
                    \Log::warning('No previous create_sell found for edit_sell_reversal', [
                ...
                // Get the latest stock entry for the product, variation, and location
                $latest_stock = CurrentStockBackup::where('business_id', $business_id)
                    ->where('product_id', $product['product_id'])
                    ->where('variation_id', $product['variation_id'])
                    ->where('location_id', $location_id)
                    ->orderBy('transaction_date', 'desc')
                    ->orderBy('id', 'desc') // Ensure we get the latest entry
                    ->first();
    
                // Calculate new_qty for edit_sell_reversal
                $new_qty = $latest_stock ? $latest_stock->new_qty + $quantity : $quantity;
            } elseif ($t_type == 'delete_product_sell') {
                // For delete_product_sell, use the quantity from the original sell line (already in base unit)
                $quantity = $product['quantity']; // Quantity is already in base unit from original sell line
                ...
                // Calculate new_qty by adding back the deleted quantity
                $new_qty = $latest_stock ? $latest_stock->new_qty + $quantity : $quantity;
            }
            ...
            $stock_line = [
                'business_id' => $business_id,
                'location_id' => $location_id, // Add location_id to the stock backup
                'product_id' => $product['product_id'],
                'variation_id' => $product['variation_id'],
                'transaction_id' => $transaction->id,
                't_type' => $t_type,
                'qty_change' => $quantity, // Store as positive (in base unit)
    ...
    private function __makeLinesForComboProduct($combo_items, $parent_sell_line, $combo, $qtyforSingle)
    {
        $combo_lines = [];
        $combo_total_price = 0;
  
        // Step 1: Calculate total price for percentage change
        ...
        // Step 2: Save Main Combo Product
        foreach ($combo_items as $value) {
            $price = $this->calc_percentage($value['unit_price_inc_tax'], $change_percent, $value['unit_price_inc_tax']);
            // ✅ **First, save the main transaction sell line (as normal, no changes)**
            $sell_line = TransactionSellLine::create([
                'transaction_id' => $parent_sell_line->transaction_id,
                'product_id' => $value['product_id'],
                'variation_id' => $value['variation_id'],
                'quantity' => $combo == 'combo_single' ? $value['quantity'] : $value['quantity'],
                ...
                'children_type' => $combo == 'combo_single' ? 'combo_single' : 'combo',
            ]);

            $combo_lines[] = $sell_line; // Store sell line for reference

            // ✅ **Step 3: After saving, check if product is combo or combo_single**
            $product = Product::find($value['product_id']);

            if ($product && in_array($product->type, ['combo', 'combo_single'])) {
                // ✅ Query Variations Table to Get `combo_variations`
                $product_variations = Variation::where('product_id', $value['product_id'])->get();
               
                foreach ($product_variations as $product_variation) {
                    
                    if (!empty($product_variation->combo_variations)) {
                        
                        // ✅ Fix: Ensure `combo_variations` is a string before decoding
                        $combo_variations = is_array($product_variation->combo_variations)
                        ? $product_variation->combo_variations  // Already an array, use as is
                        : json_decode($product_variation->combo_variations, true); // Decode JSON
                       
                        foreach ($combo_variations as $variation) {
                            $variation_data = Variation::find($variation['variation_id']);
                            if ($variation_data) {
                                // ✅ Find the actual product linked to this variation
                                $nested_product = Product::find($variation_data->product_id);

                                if ($nested_product) {
                                    $unit_price_inc_tax = $variation_data->sell_price_inc_tax;

                                    // ✅ Save Nested Combo Products (AFTER main sell_line is saved)
                                    $nested_sell_line = TransactionSellLine::create([
                                        'transaction_id' => $parent_sell_line->transaction_id,
                                        'product_id' => $nested_product->id,
                                        'variation_id' => $variation['variation_id'],
                                        'quantity' => $variation['quantity'] * $value['quantity'],
                                        'unit_price_before_discount' => $unit_price_inc_tax,
                                        ...
                                        'sub_unit_id' => $variation['unit_id'] ?? null,
                                        'discount_id' => null,
                                        'parent_sell_line_id' => $sell_line->id, // ✅ Link to the just-saved `sell_line`
                                        'children_type' => 'combo',
                                    ]);

                                    $combo_lines[] = $nested_sell_line;
                                }
                            }
                        }
                    }
    ...
    public function editSellLine($product, $location_id, $status_before, $multiplier = 1, $uf_data = true)
    {
        ...
        if ($product['product_type'] == 'combo_single') {
            //$this->editSellLineCombo($sell_line, $location_id, $sell_line->quantity, $new_qty);
            //Assign combo product sell line to $edit_ids so that it will not get deleted
            foreach ($product['combo_single'] as $combo_line) {
                $edit_ids[] = $combo_line['transaction_sell_lines_id'];
            }
    ...
    public function deleteSellLines($transaction_line_ids, $location_id, $transaction_type = 'sell', $force_delete = true)
    {
        if (!empty($transaction_line_ids)) {
            ...
            // **This line now only executes if force_delete is true**
            if ($force_delete) {
    ...
    public function createOrUpdatePaymentLines($transaction, $payments, $business_id = null, $user_id = null, $uf_data = true)
    {
        ...
        // Track cash ring payments for processing
        $cash_ring_payments = [];
        ...
        foreach ($payments as $payment) {
            ...
            if (! empty($payment['payment_id'])) {
                ...
                if ($payment_amount != 0) {
                    ...
                    // Handle cash ring percentage payment - SAVE FINAL AMOUNT as payment amount
                    $final_payment_amount = $payment_amount; // Default to original amount
                    ...
                    if ($payment['method'] == 'cash_ring_percentage') {
                        ...
                        $final_amount = isset($payment['cash_ring_final_amount']) ? $this->num_uf($payment['cash_ring_final_amount']) : 0;
                        
                        // IMPORTANT: Save the FINAL AMOUNT as the payment amount in database
                        $final_payment_amount = $final_amount;
                        
                        // Track for cash ring balance processing - use ORIGINAL amount for balance deduction
                        $cash_ring_payments[] = [
                            'contact_id' => $transaction->contact_id,
                            'business_id' => $transaction->business_id,
                            'amount' => $payment_amount, // Original amount for balance calculation
                    ...
                    $payment_data = [
                        'amount' => $final_payment_amount, // Use final amount for cash_ring_percentage, original amount for others
                    ...
                    // Store additional cash ring data in payment record
                    if ($payment['method'] == 'cash_ring_percentage') {
                        // Save original amount (before percentage calculation) in cash_ring_percentage column
                        $payment_data['cash_ring_percentage'] = $payment_amount;
                        // Save percentage value in percentage column
                        $payment_data['percentage'] = $percentage;
                    }
                    
                    // Handle regular cash ring payment
        ...
        // Process cash ring payments and update StockCashRingBalanceCustomer
        if (!empty($cash_ring_payments)) {
            $this->processCashRingPayments($cash_ring_payments);
        }
    ...
    /**
     * Process cash ring payments and update customer balance
     */
    public function processCashRingPayments($cash_ring_payments)
    {
        try {
            foreach ($cash_ring_payments as $cash_ring_payment) {
                $contact_id = $cash_ring_payment['contact_id'];
                $business_id = $cash_ring_payment['business_id'];
                $amount = $cash_ring_payment['amount'];
                $method = $cash_ring_payment['method'];
                
                // Calculate the amount to deduct from customer balance
                $amount_to_deduct = 0;
                
                if ($method == 'cash_ring_percentage') {
                    // IMPORTANT: For balance deduction, use the ORIGINAL amount (not percentage calculation)
                    $amount_to_deduct = $amount; // Use original amount for balance deduction
                    
                    \Log::info('Cash Ring Percentage Payment - Deducting ORIGINAL amount:', [
                    ...
                    // Regular cash ring payment - deduct full amount
                    $amount_to_deduct = $amount;
                    
                    \Log::info('Cash Ring Payment - Deducting:', [
                        'contact_id' => $contact_id,
                        'business_id' => $business_id,
                        'amount_to_deduct' => $amount_to_deduct
                    ]);
                ...
                $stock_balance = \App\StockCashRingBalanceCustomer::where('contact_id', $contact_id)
                    ->where('business_id', $business_id)
                    ->first();
                
                if ($stock_balance) {
                    // Update existing record - DEDUCT the amount
                    $current_dollar = $stock_balance->total_cuurency_dollar ?? 0;
                    
                    // DEDUCT amount from current balance (subtract instead of add)
                    $new_total_balance = $current_dollar - $amount_to_deduct;
                    
                    // Update the dollar amount (single currency approach)
                    $stock_balance->total_cuurency_dollar = $new_total_balance;
                    $stock_balance->save();
                    
                    \Log::info('Updated existing StockCashRingBalanceCustomer - DEDUCTED (Single Currency):', [
                        'contact_id' => $contact_id,
                        'business_id' => $business_id,
                        'previous_dollar_balance' => $current_dollar,
                        'amount_deducted' => $amount_to_deduct,
                        'new_dollar_balance' => $new_total_balance,
                        'is_negative_balance' => $new_total_balance < 0 ? 'YES' : 'NO'
                    ]);
                } else {
                    // Create new record with NEGATIVE balance (customer owes money)
                    $negative_balance = -$amount_to_deduct; // Store as negative number
                    
                    $stock_balance = new \App\StockCashRingBalanceCustomer();
                    $stock_balance->contact_id = $contact_id;
                    $stock_balance->business_id = $business_id;
                    $stock_balance->total_cuurency_dollar = $negative_balance;
                    $stock_balance->save();
                    
                    \Log::info('Created new StockCashRingBalanceCustomer with NEGATIVE balance (Single Currency):', [
                        'contact_id' => $contact_id,
                        'business_id' => $business_id,
                        'amount_deducted' => $amount_to_deduct,
            ...
            \Log::emergency('Error processing cash ring payments: ' . $e->getMessage() . ' File: ' . $e->getFile() . ' Line: ' . $e->getLine());
     ...
     * @return array
     */
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type, $business_id)
    {
        $il = $invoice_layout;
        ...
        if (in_array($transaction_type, ['sell', 'sales_order'])) {
            ...
            $details = $this->_receiptDetailsSellLines($lines, $il, $business_details, $business_id);
        ...
        $output['total_kh_label'] = $il->total_kh_label;
        ...
        $output['payment_lable'] = $invoice_layout->payment_lable ?? " ";

        if($transaction->payment_status == 'paid'){
            $output['payment_value'] = $invoice_layout->payment_paid ?? " ";
        }elseif($transaction->payment_status == 'due'){
            $output['payment_value'] = $invoice_layout->payment_due ?? " ";
        }elseif($transaction->payment_status == 'partial'){
            $output['payment_value'] = $invoice_layout->payment_partial ?? " ";
        }else{
        ...
        $output['total_exchange_rate'] = number_format($transaction->exchange_rate * $transaction->final_total, 0, '.', ',');
        $output['exchange_rate'] = number_format($transaction->exchange_rate, 0, '.', ',');
        ...
        if (in_array($transaction_type, ['sell', 'sales_order'])) {
            ...
            if ($zatca_qr) {
                ...
                if (in_array('piikmall_invoice', $qr_code_fields)) {
                    $qr_code_details[] = $transaction->full_invoice_pk;
                }
     ...
     * @return array
     */
    protected function _receiptDetailsSellLines($lines, $il, $business_details, $business_id)
    {
        $is_lot_number_enabled = $business_details->enable_lot_number;
        $is_product_expiry_enabled = $business_details->enable_product_expiry;
        ...
        foreach ($lines as $line) {
            ...
            $base_unit_multiplier_check = null;

            // Check if the base_unit_id is null and find a unit where the short_name matches the current unit's short_name
            if (!empty($line->sub_unit_id)) {
                $matching_unit = Unit::where('id', $line->sub_unit_id)
                                     ->where('business_id', $business_id)
                                     ->first();
                if ($matching_unit) {
                    $base_unit_multiplier_check =$matching_unit->base_unit_multiplier; // Convert to number
            ...
            // Check if $base_unit_multiplier_check is not empty and not zero before division
            if (empty($unit->base_unit_id) && !empty($base_unit_multiplier_check) && $base_unit_multiplier_check != 0) {
                $convert_quantity = $line->quantity / $base_unit_multiplier_check;
            } else {
                // Fallback to the original quantity if base_unit_id is not empty or base_unit_multiplier_check is zero
                $convert_quantity = $line->quantity;
            }
            ...
            $line_array = [
                ...
                'quantity' => $this->num_f($convert_quantity, false, $business_details, true),
                ...
                'base_unit_multiplier_check' => $base_unit_multiplier_check, // Added this field
    ...
    public function deleteSale($business_id, $transaction_id)
    {
        ...
        $this->activityLog($transaction, $log_type, null, $log_properties);

        // If status is draft, directly delete transaction
        if ($transaction->status == 'draft') {
            $transaction_payments = $transaction->payment_lines;
            
            // Process cash ring reversal BEFORE deleting payments (only for sell, not sales_order)
            if (!empty($transaction_payments) && !empty($transaction->contact_id) && $transaction->type == 'sell') {
                $this->reverseCashRingBalance($transaction_payments, $transaction->contact_id, $business_id);
            }
            ...
            // Handle stock backup for delete_sell (only for regular sell, not sales_order)
            if ($transaction->type == 'sell') {
                $stock_lines = [];
            ...
            // Delete Cash register transactions (only for sell)
            if ($transaction->type == 'sell') {
            ...
            // Fire payment deleted events (only for sell)
            if ($transaction->type == 'sell') {
    ...
    /**
     * Reverse cash ring balance for customer when deleting transaction
     */
    private function reverseCashRingBalance($payments, $contact_id, $business_id)
    {
        $total_reversal_amount = 0;

        foreach ($payments as $payment) {
            if (in_array($payment->method, ['cash_ring_percentage', 'cash_ring'])) {
                $reversal_amount = 0;

                if ($payment->method == 'cash_ring') {
                    // For cash_ring, get value from amount column
                    $reversal_amount = floatval($payment->amount);
                } elseif ($payment->method == 'cash_ring_percentage') {
                    // For cash_ring_percentage, get value from cash_ring_percentage column
                    $reversal_amount = floatval($payment->cash_ring_percentage);
                }
        ...
        // If there's amount to reverse, update the customer balance
        if ($total_reversal_amount > 0) {
            // Use DB::table for direct database update to avoid model issues
            $existing_balance = \DB::table('stock_cash_ring_balance_customer')
                ->where('contact_id', $contact_id)
                ->where('business_id', $business_id)
                ->first();

            if ($existing_balance) {
                // Update existing record - add back the amount (reversal)
                \DB::table('stock_cash_ring_balance_customer')
                    ->where('contact_id', $contact_id)
                    ->where('business_id', $business_id)
                    ->update([
                        'total_cuurency_dollar' => $existing_balance->total_cuurency_dollar + $total_reversal_amount
                    ]);
            } else {
                // Insert new record with the reversal amount
                \DB::table('stock_cash_ring_balance_customer')
                    ->insert([
                        'contact_id' => $contact_id,
                        'business_id' => $business_id,
                        'total_cuurency_dollar' => $total_reversal_amount
                    ]);
            }

            // Log the reversal for debugging
            \Log::info("Cash ring reversal: contact_id={$contact_id}, business_id={$business_id}, amount={$total_reversal_amount}");
        }
    }
...
public function getListSells($business_id, $sale_type = 'sell')
    {
        $sells = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
            ...
            ->leftJoin('transactions as so_trans', function($join) {
                $join->whereRaw('FIND_IN_SET(so_trans.id, REPLACE(REPLACE(REPLACE(transactions.sales_order_ids, "[", ""), "]", ""), "\"", ""))')
                    ->where('so_trans.type', 'sales_order');
            })
            ->leftJoin('salepro.products as sp', 'tsl.product_id', '=', 'sp.id') // Join with products table
            ->where('transactions.business_id', $business_id)
            ...
            ->select(
                ...
                'so_trans.invoice_no as sales_order_invoice',
                // Add new calculation for Product Count KPI
                DB::raw('SUM(CASE WHEN sp.product_kpi = 2 THEN tsl.quantity ELSE 0 END) as product_count_kpi')
            );
    ...
    public function addSellReturn($input, $business_id, $user_id, $uf_number = true, $location_id = null)
    {
        $discount = [
        ...
        if (empty($sell_return)) {
            ...
            $sell_return_data['location_id'] = $location_id ?? $sell->location_id;
            ...
            // Verify location_id matches the parent sale's location_id
            if ($location_id && $location_id != $sell->location_id) {
                throw new \Exception('Location mismatch: The return location must match the original sale location.');
            }
        ...
        // Prepare stock backup for create_sell_return and reversal if editing
        $stock_lines = [];
        foreach ($sell->sell_lines as $sell_line) {
            if (array_key_exists($sell_line->id, $returns)) {
                ...
                // If editing an existing sell return, reverse the previous create_sell_return
                if (!empty($sell_return_before)) {
                    // Find the existing create_sell_return record for this transaction_id, product_id, and location_id
                    $previous_sell_return = CurrentStockBackup::where('business_id', $business_id)
                        ->where('product_id', $sell_line->product_id)
                        ->where('location_id', $sell_return->location_id)
                        ->where('transaction_id', $sell_return->id)
                        ->where('t_type', 'create_sell_return')
                        ->orderBy('transaction_date', 'desc')
                        ->orderBy('id', 'desc')
                        ->first();

                    if ($previous_sell_return) {
                        \Log::info("Found previous create_sell_return for transaction_id {$sell_return->id}, product_id {$sell_line->product_id}, location_id {$sell_return->location_id}: " . json_encode($previous_sell_return->toArray()));

                        // Fetch the last new_qty before the previous create_sell_return
                        $last_stock_before_previous = CurrentStockBackup::where('business_id', $business_id)
                            ->where('product_id', $sell_line->product_id)
                            ->where('location_id', $sell_return->location_id)
                            ->where('id', '<', $previous_sell_return->id)
                            ->orderBy('transaction_date', 'desc')
                            ->orderBy('id', 'desc')
                            ->first();

                        $current_stock = $last_stock_before_previous ? $last_stock_before_previous->new_qty : 0;
                        \Log::info("Last new_qty before previous create_sell_return for product_id {$sell_line->product_id} at location_id {$sell_return->location_id}: $current_stock");

                        // Create a reversal record (edit_sell_return_reversal)
                        $reversal_stock_line = [
                            'business_id' => $business_id,
                            ...
                            't_type' => 'edit_sell_return_reversal',
                            'qty_change' => $previous_sell_return->qty_change,
                            'new_qty' => $current_stock,
                            'transaction_date' => now(),
                        ];

                        $stock_lines[] = new CurrentStockBackup($reversal_stock_line);
                        \Log::info("Created edit_sell_return_reversal for transaction_id {$sell_return->id}, product_id {$sell_line->product_id}, location_id {$sell_return->location_id}: " . json_encode($reversal_stock_line));
                    }
                }

                // If no reversal was needed, fetch the last new_qty from the database
                if (empty($sell_return_before) || !$previous_sell_return) {
                    $latest_stock = CurrentStockBackup::where('business_id', $business_id)
                        ->where('product_id', $sell_line->product_id)
                        ->where('location_id', $sell_return->location_id)
                        ->orderBy('transaction_date', 'desc')
                    ...
                    $current_stock = $latest_stock ? $latest_stock->new_qty : 0;
                    \Log::info("Last new_qty for product_id {$sell_line->product_id} at location_id {$sell_return->location_id}: $current_stock");
                }

                // Calculate new_qty (add the returned quantity to increase stock)
                $new_qty = $current_stock + $quantity;
                \Log::info("Calculated new_qty for create_sell_return: $new_qty (current_stock: $current_stock + quantity: $quantity)");

                // Prepare stock backup line for create_sell_return with location_id
                $stock_line = [
                    'business_id' => $business_id,
                    'location_id' => $sell_return->location_id,
                    'product_id' => $sell_line->product_id,
        ...
        // Save the stock backup lines (reversal and new create_sell_return)
        if (!empty($stock_lines)) {
            \Log::info("Saving stock lines: " . json_encode($stock_lines));
            CurrentStockBackup::insert(
                array_map(function ($line) {
                    return $line->toArray();
                }, $stock_lines)
            ...
            \Log::warning("No stock lines to save for transaction_id {$sell_return->id}");
        }
