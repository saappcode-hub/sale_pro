...
class ProductUtil extends Util
{
    ...
    //     }
        // }
    public function decreaseProductQuantityCombo($combo_details, $location_id, $visited_products = [])
    {
        foreach ($combo_details as $details) {
            $product_id = $details['product_id'];
            // Prevent circular references
            if (in_array($product_id, $visited_products)) {
                throw new \Exception('Circular combo reference detected for product ID: ' . $product_id);
            }
            $visited_products[] = $product_id;

            // Step 1: Decrease stock for each product in the combo
            $this->decreaseProductQuantity(
                $details['product_id'],
                $details['variation_id'],
                $location_id,
            ...
            // Step 2: Check if this product is also a combo or combo_single and decrease its nested products
            $product = Product::find($details['product_id']);
            if ($product && in_array($product->type, ['combo', 'combo_single'])) {
                $product_variations = Variation::where('product_id', $details['product_id'])->get();
                foreach ($product_variations as $product_variation) {
                    if (!empty($product_variation->combo_variations)) {
                        $combo_variations = is_array($product_variation->combo_variations)
                            ? $product_variation->combo_variations
                            : json_decode($product_variation->combo_variations, true);

                        foreach ($combo_variations as $variation) {
                            $variation_data = Variation::find($variation['variation_id']);
                            if ($variation_data) {
                                // Step 3: Decrease stock for nested products
                                $this->decreaseProductQuantity(
                                    $variation_data->product_id,
                                    $variation['variation_id'],
                                    $location_id,
                                    $variation['quantity'] * $details['quantity']
                                );
                            }
                        }
                    }
    ...
    public function decreaseProductQuantitySingle($product_id, $variation_id, $location_id, $new_quantity, $old_quantity = 0)
    {
        $qty_difference = $new_quantity - $old_quantity;
     ...
     * @return void
     */
    public function decreaseProductQuantityComboSingle($combo_details, $location_id)
    {
        //variation id is child product variation id
        foreach ($combo_details as $details) {
            $this->decreaseProductQuantitySingle(
                $details['product_id'],
                $details['variation_id'],
                $location_id,
                $details['quantity']
    ...
    public function getDetailsFromVariation($variation_id, $business_id, $location_id = null, $check_qty = true)
    {
        ...
        if ($product->product_type == 'combo' || $product->product_type == 'combo_single') {
            if ($check_qty) {
    ...
    {
        // 🚫 Skip stock adjustment if transaction type is 'sales_order'
        if (isset($transaction->type) && $transaction->type == 'sales_order') {
            return;
        }
        ...
        if ($status_before == 'final' && $transaction->status == 'draft') {
            foreach ($input['products'] as $product) {
                if (!empty($product['transaction_sell_lines_id'])) {
                    ...
                    if (isset($product['product_type']) && in_array($product['product_type'], ['combo', 'combo_single'])) {
                        $combo_details = $product['product_type'] == 'combo'
                            ? $product['combo']
                            : $product['combo_single'];
                        $this->updateProductQuantityCombo($combo_details, $input['location_id']);
                    }
                }
            }
    ...
    public function updateProductQuantityCombo($combo_details, $location_id, $visited_products = [])
    {
        foreach ($combo_details as $details) {
            $product_id = $details['product_id'];
            if (in_array($product_id, $visited_products)) {
            ...
            // Increase stock for each product in the combo
            $this->updateProductQuantity(
                $location_id,
                $details['product_id'],
                $details['variation_id'],
    ...
    public function createOrUpdatePurchaseLines($transaction, $input_data, $currency_details, $enable_product_editing, $before_status = null)
    {
        ...
        // For update case: Fetch original purchase lines to detect new, updated, and deleted products
        $original_purchase_lines = [];
        if ($before_status !== null) { 
            $original_purchase_lines = PurchaseLine::where('transaction_id', $transaction->id)
                ->get()
                ->mapWithKeys(function ($line) {
                    return [$line->product_id . '_' . $line->variation_id => [
                        ...
                        'base_unit_multiplier' => !empty($line->sub_unit_id) ? Unit::find($line->sub_unit_id)->base_unit_multiplier : 1,
                    ]];
                })->toArray();
        }
        ...
        foreach ($input_data as $data) {
            ...
            // =========================================================
            // [UPDATED LOGIC] Check if stock update should be skipped
            // We only skip if this specific product triggers a Supplier Reward
            // =========================================================
            $skip_stock_update = false;
            
            if (!empty($data['purchase_type_id'])) {
                $pType = PurchaseType::find($data['purchase_type_id']);
                
                // Check if Purchase Type has Exchange Products configured
                if ($pType && !empty($pType->exchange_product_id) && is_array($pType->exchange_product_id)) {
                    
                    // Loop through the Exchange IDs in the Purchase Type
                    foreach($pType->exchange_product_id as $ex_prod_id) {
                        
                        // Check if a rule exists in rewards_exchange for THIS product
                        $hasRule = RewardsExchange::where('exchange_product', $ex_prod_id)
                            ->where('receive_product', $data['product_id']) // Match the product being purchased
                        ...
                        if ($hasRule) {
                            ...
                            // If we find one valid rule for this product, we skip stock update 
                            // (because the Reward System will handle it) and stop looking.
                            break; 
                        }
                    }
                }
            ...
            if (isset($data['purchase_line_id'])) {
                ...
                if ($transaction->status == 'received' && $before_status !== null) {
                    ...
                    if (isset($original_purchase_lines[$key])) {
                        $original_quantity = $original_purchase_lines[$key]['quantity'];
                        ...
                        unset($original_purchase_lines[$key]);
                    }
                }
            } else {
            ...
            $purchase_line->purchase_type_id = !empty($data['purchase_type_id']) ? $data['purchase_type_id'] : null;
        ...
        // Handle stock backups
        if ($transaction->status == 'received' && $transaction->type == 'purchase') {
            $business_id = $transaction->business_id;
    
            // Step 1: New products (create_purchase)
            if (!empty($new_products)) {
                $this->saveCurrentPurchaseStockBackup(
                    $transaction,
                    $new_products,
                    $business_id,
            ...
            // Step 2: Updated products (edit_purchase_reversal and create_purchase)
            ...
            // Step 3: Deleted products (delete_product_purchase)
            $deleted_products = array_values($original_purchase_lines);
        ...
        if (!empty($updated_purchase_line_ids)) {
            ...
            if ($delete_purchase_lines->count()) {
                foreach ($delete_purchase_lines as $delete_purchase_line) {
                    ...
                    // Check if this deleted line should also skip stock update
                    $should_decrease = true;
                    if (!empty($delete_purchase_line->purchase_type_id)) {
                        $pType = PurchaseType::find($delete_purchase_line->purchase_type_id);
                        
                        // Use the SAME logic: if a rule exists, we skipped it before, so we skip decreasing it now
                        if ($pType && !empty($pType->exchange_product_id) && is_array($pType->exchange_product_id)) {
                            foreach($pType->exchange_product_id as $ex_prod_id) {
                                $hasRule = RewardsExchange::where('exchange_product', $ex_prod_id)
                                    ->where('receive_product', $delete_purchase_line->product_id)
                                    ->where('type', 'suppliers')
                                    ->where('business_id', $transaction->business_id)
                                    ->whereNull('deleted_at')
                                    ->exists();
                                ...
                                if ($hasRule) {
                                    $should_decrease = false; // It was a reward product, so we never added stock. Don't decrease.
                                    break; 
                                }
                            }
                        }
                    ...
                    // If purchase requisition line set decrease quantity
    ...
    public function saveCurrentPurchaseStockBackup($transaction, $products, $business_id, $location_id, $uf_data = true, $t_type = 'create_purchase')
    {
        $stock_lines = [];
        ...
        foreach ($products as $product) {
            ...
            if (!empty($product['sub_unit_id'])) {
                $unit = Unit::find($product['sub_unit_id']);
                $multiplier = !empty($unit->base_unit_multiplier) ? $unit->base_unit_multiplier : 1;
            }

            // Calculate quantity (positive for purchase)
            $uf_quantity = $uf_data ? $this->num_uf($product['quantity']) : $product['quantity'];
            $quantity = $uf_quantity * $multiplier; // Positive quantity in base unit

            // Initialize new_qty
            $new_qty = 0;

            if ($t_type == 'create_purchase') {
                // For create_purchase, get the last new_qty for the product_id, variation_id, and location_id
                $latest_stock = CurrentStockBackup::where('business_id', $business_id)
                    ->where('product_id', $product['product_id'])
                    ->where('variation_id', $product['variation_id'])
                    ->where('location_id', $location_id) // Verify location_id
                    ->orderBy('transaction_date', 'desc')
                    ->orderBy('id', 'desc') // Ensure we get the latest record
                    ->first();

                $current_stock = $latest_stock ? $latest_stock->new_qty : 0;
                $new_qty = $current_stock + $quantity; // Sum only if location_id matches
            } elseif ($t_type == 'edit_purchase_reversal') {
                // For edit_purchase_reversal, fetch the last create_purchase for this transaction_id, product_id, variation_id, and location_id
                $last_create_purchase = CurrentStockBackup::where('business_id', $business_id)
                    ->where('product_id', $product['product_id'])
                    ->where('variation_id', $product['variation_id'])
                    ->where('location_id', $location_id) // Verify location_id
                    ->where('transaction_id', $transaction->id)
                    ->where('t_type', 'create_purchase')
                    ->orderBy('transaction_date', 'desc')
                    ->orderBy('id', 'desc') // Ensure we get the latest create_purchase
                    ->first();

                if ($last_create_purchase) {
                    $quantity = $last_create_purchase->qty_change; // Use the original quantity from the last create_purchase
                    $latest_stock = CurrentStockBackup::where('business_id', $business_id)
                        ->where('product_id', $product['product_id'])
                        ->where('variation_id', $product['variation_id'])
                        ->where('location_id', $location_id) // Verify location_id
            ...
            } elseif ($t_type == 'delete_product_purchase') {
                // For delete_product_purchase, use the quantity from the original purchase line
                $quantity = $product['quantity']; // Quantity is already in base unit from original purchase line
                $latest_stock = CurrentStockBackup::where('business_id', $business_id)
                    ->where('product_id', $product['product_id'])
                    ->where('variation_id', $product['variation_id'])
                    ->where('location_id', $location_id) // Verify location_id
    ...
    public function getProductStockDetails($business_id, $filters, $for)
    {
        $query = Variation::join('products as p', 'p.id', '=', 'variations.product_id')
                  ...
                  ->leftJoin('units as purchase_units', 'p.purchase_unit', '=', 'purchase_units.id')
                  ->leftjoin('variation_location_details as vld', 'variations.id', '=', 'vld.variation_id')
    ...
    public function __getComboProductDetails($combo_variations, $business_id)
    {
        foreach ($combo_variations as $key => $value) {
            ...
            $product_unit_id = $combo_variations[$key]['variation']['product']->unit_id ?? null;
            $combo_variations[$key]['sub_units'] = $product_unit_id ? $this->getSubUnits($business_id, $product_unit_id, true) : [];
    
            $combo_variations[$key]['multiplier'] = 1;
    
            // Ensure 'units' exists before accessing it
            if (!empty($combo_variations[$key]['sub_units']['units']) && isset($combo_variations[$key]['sub_units']['units'][$combo_variations[$key]['unit_id']])) {
                $unit = $combo_variations[$key]['sub_units']['units'][$combo_variations[$key]['unit_id']];
                $combo_variations[$key]['multiplier'] = $unit['multiplier'] ?? 1;
                $combo_variations[$key]['unit_name'] = $unit['name'] ?? 'N/A';
            }
        }
...
// ─────────────────────────────────────────────────────────────────────────────
// METHOD 1 — getVariationStockDetails (same as before, no changes)
// ─────────────────────────────────────────────────────────────────────────────
public function getVariationStockDetails($business_id, $variation_id, $location_id)
{
    $variation_info = Variation::join('products as p', 'p.id', '=', 'variations.product_id')
        ->join('units', 'p.unit_id', '=', 'units.id')
        ->leftJoin('units as u', 'p.secondary_unit_id', '=', 'u.id')
        ...
        ->where('variations.id', $variation_id)
        ->select(
            'variations.sub_sku as sub_sku', 'p.name as product', 'p.type', 'p.sku',
            'p.id as product_id', 'units.short_name as unit', 'u.short_name as second_unit',
            'pv.name as product_variation', 'variations.name as variation_name', 'variations.id as variation_id'
        )->first();
    ...
    $purchase_agg = DB::table('purchase_lines as pl')
        ->join('transactions as t', function($j) use($location_id) {
            $j->on('pl.transaction_id','=','t.id')->where('t.location_id',$location_id)->whereNull('t.deleted_at');
        })->leftJoin('purchase_types as pt','pl.purchase_type_id','=','pt.id')
        ->where('pl.variation_id',$variation_id)
        ->select(
            DB::raw("COALESCE(SUM(CASE WHEN t.type='purchase' AND t.status='received' AND (pt.exchange_product_id IS NULL OR JSON_CONTAINS(pt.exchange_product_id,'null')) THEN pl.quantity ELSE 0 END),0) as total_purchase"),
            DB::raw("COALESCE(SUM(CASE WHEN t.type IN('purchase','purchase_return') THEN pl.quantity_returned ELSE 0 END),0) as total_purchase_return"),
            DB::raw("COALESCE(SUM(pl.quantity_adjusted),0) as total_adjusted"),
            DB::raw("COALESCE(SUM(CASE WHEN t.type='opening_stock' AND t.status='received' THEN pl.quantity ELSE 0 END),0) as total_opening_stock"),
    ...
    $adj_agg = DB::table('stock_adjustment_lines as al')
        ->join('transactions as t', function($j) use($location_id) {
            $j->on('al.transaction_id','=','t.id')->where('t.location_id',$location_id)->where('t.type','stock_adjustment')->whereNull('t.deleted_at');
        })->where('al.variation_id',$variation_id)
        ->select(DB::raw("COALESCE(SUM(al.quantity),0) as total_adjusted"))->first();
    ...
    $supplier_reward_exchange = (float)DB::table('stock_reward_exchange_new as sre')
        ->join('transactions as t', function($j) use($location_id) {
            $j->on('t.id','=','sre.transaction_id')->where('t.location_id',$location_id)->where('t.sub_type','send')
              ->where('t.type','supplier_exchange')->whereNull('t.deleted_at')->whereNull('t.deleted_by');
        })->where('sre.variation_id',$variation_id)->where('sre.type','supplier_reward_out')->whereNull('sre.contact_id')->sum('sre.quantity');

    $supplier_reward_exchange_receive = (float)DB::table('stock_reward_exchange_new as sre')
        ->join('transactions as t', function($j) use($location_id) {
            $j->on('t.id','=','sre.transaction_id')->where('t.location_id',$location_id)
              ->where('t.type','supplier_exchange_receive')->whereNull('t.deleted_at')->whereNull('t.deleted_by');
        })->where('sre.variation_id',$variation_id)->where('sre.type','supplier_receive_in')->whereNull('sre.contact_id')->sum('sre.quantity');

    $product_name = $variation_info->type=='variable'
...
// ─────────────────────────────────────────────────────────────────────────────
// METHOD 2 — getVariationStockHistory (FAST: 1 query per page change)
// ─────────────────────────────────────────────────────────────────────────────
//
// SPEED FIX: accepts $cached_total and $page_start_stock from JS.
// - Page 1 (initial): runs COUNT + data query = 2 queries
// - Page 2+ (clicks): runs ONLY data query = 1 query (100-300ms)
// - "Show entries" change: 1 query
//
public function getVariationStockHistory(
    $business_id,
    $variation_id,
    $location_id,
    $current_stock = null,
    ...
    // ── Build the UNION ALL SQL ──────────────────────────────────────────
    $union_sql = $this->buildStockHistoryUnionSql($vid, $lid);

    // ── COUNT: only run if not cached ────────────────────────────────────
    if ($cached_total !== null && $cached_total >= 0) {
        $total = (int) $cached_total;
    } else {
        $count_result = DB::select("SELECT COUNT(*) as total FROM ({$union_sql}) as su");
        $total = $count_result[0]->total ?? 0;
    }

    if ($total == 0) {
        return ['data'=>[],'total'=>0,'page'=>1,'per_page'=>$per_page,'last_page'=>1];
    }
    ...
    foreach ($rows as $row) {
        ...
        $entry = [
            'date'=>$row->transaction_date, 'transaction_id'=>$row->transaction_id,
            'contact_name'=>$row->contact_name??'--', 'supplier_business_name'=>$row->supplier_business_name??'',
            'quantity_change'=>$qty, 'type'=>$row->transaction_type,
            'type_label'=>$this->getStockHistoryTypeLabel($row->transaction_type, $row->source_type, $qty),
            'ref_no'=>$ref_no??'', 'additional_notes'=>$row->additional_notes??'',
            'stock'=>$this->roundQuantity($running_stock),
            'purchase_secondary_unit_quantity'=>0, 'sell_secondary_unit_quantity'=>0, 'stock_in_second_unit'=>0,
        ];
        if ($sec != 0) {
            if ($qty > 0) $entry['purchase_secondary_unit_quantity'] = $this->roundQuantity($sec);
            else $entry['sell_secondary_unit_quantity'] = $this->roundQuantity(abs($sec));
        }
        $result[] = $entry;
        $running_stock -= $qty;
    }
    ...
    return [
        ...
        'last_page'=>$total > 0 ? (int)ceil($total/$per_page) : 1,
        // NEW: send the stock value AFTER the last row on this page
        // JS uses this as page_start_stock for the NEXT page
        'next_page_stock'=>$this->roundQuantity($running_stock),
    ];
}
...
// ─────────────────────────────────────────────────────────────────────────────
// METHOD 3 — getStockHistoryTypeLabel (helper)
// ─────────────────────────────────────────────────────────────────────────────
private function getStockHistoryTypeLabel($transaction_type, $source_type, $qty_change)
{
    switch ($transaction_type) {
        ...
        case 'supplier_exchange': return 'Supplier Reward';
        case 'supplier_exchange_receive': return 'Supplier Reward Receive';
        default: return $transaction_type;
    }
}
