...
class UtilBackup
{
    ...
    public function getSubUnits($business_id, $unit_id, $return_main_unit_if_empty = false, $product_id = null)
    {
        ...
        $sub_units = [];
        $default_selected_unit = $unit->id; // Default to the main unit if no specific sub-unit matches
    
        // Add the main unit conditionally
        if ($return_main_unit_if_empty && $unit->sub_units->isEmpty()) {
            $sub_units[$unit->id] = [
        ...
        foreach ($unit->sub_units as $sub_unit) {
            if (empty($related_sub_units) || in_array($sub_unit->id, $related_sub_units)) {
                ...
                // Set default selected unit based on `purchase_unit`
                if (!empty($product_id) && $product->purchase_unit == $sub_unit->id) {
                    $default_selected_unit = $sub_unit->id;
                }
            }
        }
    ...
    public function uploadImageToS3($request, string $file_name, string $file_type = 'image'): ?string
    {
        ...
        $business_id = request()->session()->get('user.business_id');
        if (!$business_id) {
            throw new \Exception('Business ID not found in session');
        }
        ...
        if ($file_type === 'image') {
            ...
            $allowedMimes = Config::get('constants.document_upload_mimes_types', []);
            if (!in_array($file->getClientMimeType(), array_keys($allowedMimes))) {
                throw new \Exception('Invalid document file');
            }
        }
    
        $maxSize = Config::get('constants.document_size_limit', 2 * 1024 * 1024);
        ...
        $fileName = time() . '_' . str_replace(' ', '_', pathinfo($originalName, PATHINFO_FILENAME)) . '.' . $fileExtension;
    
        // Statically set the base folder to 'salepro' and subfolder structure
        $baseFolder = 'salepro';
        $publicFolder = 'public';
        $imageFolder = 'image';
        $businessFolder = $business_id;
    
        // Construct the target folder path: salepro/public/image/business_id
        $targetFolder = implode('/', [$baseFolder, $publicFolder, $imageFolder, $businessFolder]);
    
        try {
            // Ensure the target folder exists (creates it if it doesn't)
            Storage::disk('s3')->makeDirectory($targetFolder); // 'true' recursively creates parent directories
            ...
            if ($success) {
                return $fileName; // Returns 'salepro/public/image/business_id/123456789_filename.jpg'
            }
     ...
     * @return void
     */
    public function updateEditedSellLineCombo($lines, $location_id, $adjust_stock = true, $old_parent_qty = null, $new_parent_qty = null, $product_type = 'combo')
    {
        if (empty($lines) || $old_parent_qty === null || $new_parent_qty === null || $old_parent_qty == 0) {
            return false;
        }
        ...
        foreach ($lines as $key => $line) {
            ...
            if ($prev_line) {
                // Determine the new child quantity based on product type
                $new_quantity = $line['quantity']; // Default to the input quantity (for combo)
                if ($product_type === 'combo_single') {
                    // For combo_single, scale the quantity proportionally
                    $original_child_qty_per_unit = $line['quantity'] / $old_parent_qty;
                    $new_quantity = $original_child_qty_per_unit * $new_parent_qty;
                }

                // Debug the values to confirm
                \Log::info("Updating {$product_type} child: ", [
                    'transaction_sell_lines_id' => $line['transaction_sell_lines_id'],
                    'old_parent_qty' => $old_parent_qty,
                    'new_parent_qty' => $new_parent_qty,
                    'original_child_qty' => $line['quantity'],
                    'new_quantity' => $new_quantity,
                ]);

                $difference = $prev_line->quantity - $new_quantity;
                if ($difference != 0 && $adjust_stock) {
                ...
                $prev_line->quantity = $new_quantity;
                $prev_line->save();
