...
@section('title', __('Daily Payment Report'))

@section('content')
<section class="content-header">
    <h1>{{ __('Daily Payment Report') }}</h1>
</section>
<style>
    .status-completed {
    ...
    #daily_sales_payment_report_table {
        min-width: 1300px; /* Increased minimum width for the new column */
                    ...
                    <table class="table table-bordered table-striped" id="daily_sales_payment_report_table" style="width: 100%;">
                        ...
                        <tfoot id="table-footer" style="display: none;">
                            <tr class="footer-totals">
                                ...
                                <td id="footer-total-paid">0.00</td>
                                ...
                                @if(!empty($payment_methods))
                                    ...
                                    @endforeach
                                @endif
                                <td id="footer-backdated-paid">0.00</td>
                                <td></td>
                            </tr>
                        </tfoot>
                    </table>
...
$(document).ready(function() {
    ...
    function updateTableFooter(paymentMethods, paymentMethodLabels) {
        ...
        footerHtml += '<td id="footer-total-paid">0.00</td>';
        ...
        footerHtml += '<td id="footer-backdated-paid">0.00</td>';
        footerHtml += '<td></td>'; // Empty cell for Action column
        footerHtml += '</tr>';
    ...
    function displayFooterTotals(footerTotals) {
        if (footerTotals && Object.keys(footerTotals).length > 0) {
            ...
            // Update totals
            if (footerTotals.total_paid !== undefined) {
                $('#footer-total-paid').text(__currency_trans_from_en(footerTotals.total_paid, true));
            }
            if (footerTotals.total_invoice !== undefined) {
                $('#footer-total-invoice').text(footerTotals.total_invoice);
            }
            if (footerTotals.backdated_paid !== undefined) {
                $('#footer-backdated-paid').text(__currency_trans_from_en(footerTotals.backdated_paid, true));
            }
    ...
    function initializeDataTable(paymentMethods, paymentMethodLabels) {
        ...
        // Add backdated_paid column AFTER all payment method columns
        columns.push({ 
            data: 'backdated_paid', 
            name: 'backdated_paid', 
            title: 'A/R',
        ...
        columns.push({
            ...
            render: function(data, type, row) {
                ...
                var viewUrl = "/reports/sell-payment-report" + 
                             "?paid_on=" + encodeURIComponent(row.date) + 
                             "&sell_list_filter_location_id=" + encodeURIComponent(locationId);
                
                return '<a href="' + viewUrl + '" class="btn btn-primary btn-xs" target="_blank">' +
        ...
        dailySalesReport = $('#daily_sales_payment_report_table').DataTable({
            ...
            scrollX: true,
            ajax: {
                url: "{{ route('reports.daily-payment-report') }}",
                type: 'GET',
                data: function(d) {
    ...
    // Function to update table header - Updated for new column order
    function updateTableHeader(paymentMethods, paymentMethodLabels) {
        var headerHtml = '<tr><th>Payment Date</th><th>Total Paid</th><th>Total Invoice</th>';
        
        // Add payment method columns first
        if (paymentMethods && paymentMethods.length > 0) {
            $.each(paymentMethods, function(index, method) {
        ...
        // Add backdated column after all payment methods
        headerHtml += '<th>A/R</th>';
        // Add Action column
