...
<div class="row" style="color: #000000 !important;">
	...
	@endif
	<div class="col-xs-12 text-center">
		<p style="width: 100% !important; overflow: hidden;" class="word-wrap">
			{{-- Left side details --}}
			<span class="pull-left text-left" style="width: 54%; padding-right: 5px;">
			...
			{{-- Right side details --}}
			<span class="pull-right text-left" style="width: 38%; padding-left: 5px;">
				...
				@endif
				<br>
				<b>{{'អត្រា​ប្តូ​រ​ប្រាក់: 1$ ='}}</b> {{$receipt_details->exchange_rate}} <b>{{'KHR'}}</b>

				<br>
				<strong>{{ $receipt_details->payment_lable }}:</strong> {!!$receipt_details->payment_value ?? ''!!}
			</span>
		</p>
	</div>
</div>
				...
				@forelse($receipt_details->lines as $line)
					<tr>
						<td>
							...
							@endif
                            {{$line['name']}} 
                            @if(!empty($line['variation']) && !in_array(strtolower($line['variation']), ['dummy', 'dummy']))
                                {{$line['variation']}}
                            @endif
						...
						<td class="text-right">{{$receipt_details->currency_symbol}}{{$line['unit_price_before_discount']}}</td>
						@if(!empty($receipt_details->discounted_unit_price_label))
							<td class="text-right">
							{{$receipt_details->currency_symbol}}{{$line['unit_price_inc_tax']}} 
							</td>
						@endif
						@if(!empty($receipt_details->item_discount_label))
							<td class="text-right">
						...
						@endif
						<td class="text-right">{{$receipt_details->currency_symbol}}{{$line['line_total']}}</td>
					</tr>
					@if(!empty($line['modifiers']))
						@foreach($line['modifiers'] as $modifier)
							<tr>
								<td>
		                           {{$modifier['name']}} 
		                            @if(!empty($modifier['variation']) && !in_array(strtolower($modifier['variation']), ['dummy', 'dummy']))
		                                {{$modifier['variation']}}
		                            @endif
...
<div class="row" style="color: #000000 !important;">
	...
	<div class="col-xs-6">
		@php
			$has_payments = !empty($receipt_details->payments) ||
						!empty($receipt_details->total_paid) ||
						(!empty($receipt_details->total_due) && !empty($receipt_details->total_due_label)) ||
						!empty($receipt_details->all_due);
		@endphp

		@if($has_payments)
			{{-- Display payment table when payments exist --}}
			<table class="table table-slim" style="margin-bottom: 5px;">
			...
			{{-- Add Khmer text below payment table with aligned colons --}}
			@if($receipt_details->business_id == 53)
				<table style="width: 100%; margin-top: 0px; border-collapse: separate;">
					<tr>
						<td style="padding: 3px 0; width: 54%;">- លុយរង្វាន់</td>
						<td style="padding: 3px 0; width: 5%;">:</td>
						<td style="padding: 3px 0; width: 41%; border-bottom: 1px dotted #000 !important;">&nbsp;</td>
					</tr>
					<tr>
						<td style="padding: 3px 0; width: 54%;">- ភាគរយ</td>
						<td style="padding: 3px 0; width: 5%;">:</td>
						<td style="padding: 3px 0; width: 41%; border-bottom: 1px dotted #000 !important;">&nbsp;</td>
					</tr>
					<tr>
						<td style="padding: 3px 0; width: 54%;">- លុយត្រូវទូរទាត់</td>
						<td style="padding: 3px 0; width: 5%;">:</td>
						<td style="padding: 3px 0; width: 41%; border-bottom: 1px dotted #000 !important;">&nbsp;</td>
					</tr>
					<tr>
						<td style="padding: 3px 0; width: 54%;">- ទូរទាត់តាមធនាគារ</td>
						<td style="padding: 3px 0; width: 5%;">:</td>
						<td style="padding: 3px 0; width: 41%; border-bottom: 1px dotted #000 !important;">&nbsp;</td>
					</tr>
					<tr>
						<td style="padding: 3px 0; width: 54%;">- ទូរទាត់សាច់ប្រាក់</td>
						<td style="padding: 3px 0; width: 5%;">:</td>
						<td style="padding: 3px 0; width: 41%; border-bottom: 1px dotted #000 !important;">&nbsp;</td>
					</tr>
				</table>
			@endif
		@else
			{{-- Display only Khmer text when no payments --}}
			@if($receipt_details->business_id == 53)
				<table style="width: 100%; margin-top: 0px; border-collapse: separate;">
					<tr>
						<td style="padding: 3px 0; width: 54%;">- លុយរង្វាន់</td>
        ...
        <div class="table-responsive">
          	<table class="table table-slim">
				<tbody>
					...
					<tr>
						<th>
							{{$receipt_details->total_kh_label}}
						</th>
						<td class="text-right">
							@if(!empty($receipt_details->total_kh_label))
								{{"៛".$receipt_details->total_exchange_rate}}
							@endif
						</td>
					</tr>
				</tbody>
	...
	@if(!empty($receipt_details->additional_notes))
	    <div class="col-xs-12">
	    	@if($receipt_details->business_id == 53)
				<p>បរិយាយ: {!! nl2br($receipt_details->additional_notes) !!}</p>
			@else
...
<div class="row" style="color: #000000 !important;">
	@if($receipt_details->show_barcode || $receipt_details->show_qr_code)
		<div class="@if(!empty($receipt_details->footer_text)) col-xs-12 @else col-xs-12 @endif" style="text-align: right;">
			...
			@if($receipt_details->show_qr_code && !empty($receipt_details->qr_code_text))
				<img src="data:image/png;base64,{{DNS2D::getBarcodePNG($receipt_details->qr_code_text, 'QRCODE', 4, 4, [39, 48, 54])}}">
			@endif
		</div>
	@endif
</div>