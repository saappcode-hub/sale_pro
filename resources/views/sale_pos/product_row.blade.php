...
<?php
    // Determine the default selected unit based on the multiplier
    $default_selected_unit = null;
    foreach ($sub_units as $unit_id => $unit) {
        if ($unit['multiplier'] == 1) {
            $default_selected_unit = $unit_id;
            break;
        }
    }
?>
...
@if($product->product_type == 'combo_single' && !empty($product->combo_products))
	...
	@foreach($product->combo_products as $k => $combo_product)
		...
		<input type="hidden" 
			name="products[{{$row_count}}][combo_single][{{$k}}][variation_id]"
			value="{{$combo_product['variation_id']}}">

		<input type="hidden"
			class="combo_product_qty" 
			name="products[{{$row_count}}][combo_single][{{$k}}][quantity]"
			data-unit_quantity="{{$combo_product['qty_required']}}"
			value="{{$qty_total}}">

		@if(isset($action) && $action == 'edit')
			<input type="hidden" 
				name="products[{{$row_count}}][combo_single][{{$k}}][transaction_sell_lines_id]"
				value="{{$combo_product['id']}}">
		@endif
