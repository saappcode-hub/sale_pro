...
@php
	$title = $transaction->type == 'sales_order' ? __('lang_v1.edit_sales_order') : __('Create Invoice');
@endphp
@section('title', $title)
				...
				<div class="col-md-3">
		          <div class="form-group">
		            <div class="multi-input">
		              ...
		              <br/>
                        {!! Form::number('pay_term_number', $transaction->pay_term_number ?? 0, ['class' => 'form-control width-40 pull-left', 'placeholder' => __('contact.pay_term'), 'required']); !!}

                        {!! Form::select('pay_term_type', 
                            ['months' => __('lang_v1.months'), 
                            'days' => __('lang_v1.days')], 
                            $transaction->pay_term_type ?? 'days', 
				...
				@else
					<div class="@if(!empty($commission_agent)) col-sm-3 @else col-sm-4 @endif">
						<div class="form-group">
							...
							{!! Form::select('status', $statuses, 'final', [
...
@section('javascript')
    ...
    <script type="text/javascript">
    	$(document).ready( function(){
            ...
            // Ensure defaults for pay_term_number
            if ($('#pay_term_number').val() === '') {
                $('#pay_term_number').val(0);
            }

            // Ensure defaults for pay_term_type
            if ($('#pay_term_type').val() === '') {
                $('#pay_term_type').val('days').trigger('change');
            }

            // Ensure default status is 'final'
            if ($('#status').val() === '') {
                $('#status').val('final').trigger('change');
            }
    	});
    </script>
@endsection
