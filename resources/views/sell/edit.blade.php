...
<input type="hidden" id="transaction_back_date" value="{{ $business_details->transaction_back_date ?? 0 }}">
<input type="hidden" id="original_transaction_date" value="{{ $transaction->transaction_date }}">
	...
	<div class="row">
		<div class="col-md-12 col-sm-12">
			@component('components.widget', ['class' => 'box-solid'])
					...
					<div class="col-md-4 col-sm-6">
						<div class="form-group">
							<div class="input-group">
								...
								{!! Form::text('price_group_text', $transaction->price_group->name, ['class' => 'form-control', 'readonly' => true, 'disabled' => true, 'style' => 'background-color: #f5f5f5; color: #666; cursor: not-allowed;']); !!}
								<span class="input-group-addon">
				...
				<div class="col-sm-3">
					<div class="form-group">
						{!! Form::label('exchange_rate', __('Exchange Rate(KHR)') . ':') !!}
						{!! Form::number('exchange_rate', $exchange_rate, ['class' => 'form-control', 'id' => 'exchange_rate', 'step' => '1']); !!}
					</div>
				</div>
						...
						<tbody>
							@foreach($sell_details as $sell_line)
								...
								@include('sale_pos.product_row', [
									...
									'default_selected_unit' =>!empty($sell_line->default_selected_unit) ? $sell_line->default_selected_unit : [], 
									'action' => 'edit', 'is_direct_sell' => true, 
									'so_line' => $sell_line->so_line, 'is_sales_order' => $transaction->type == 'sales_order'])
							@endforeach
						</tbody>
					</table>
					</div>
...
@section('javascript')
    ...
    <script type="text/javascript">
    	$(document).ready( function(){
            ...
            // Update Total Riel when Total USD or Exchange Rate changes
			function updateTotalRiel() {
				var totalUsd = parseFloat($('.price_total').text().replace(/[^0-9.-]+/g,"")) || 0;
				var exchangeRate = parseFloat($('#exchange_rate').val()) || 0;
				var totalRiel = totalUsd * exchangeRate;
				// Display Total Riel as a whole number without decimals
				$('.total_riel').text(totalRiel.toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 0 }));
			}

			// Update Total Riel on page load
			updateTotalRiel();

			// Update Total Riel when Exchange Rate changes
			$('#exchange_rate').on('input change', function() {
				updateTotalRiel();
			});

			// Update Total Riel when Total USD changes
			// We need to observe price_total as it's dynamically updated
			...
			if (priceTotalElement) {
				observer.observe(priceTotalElement, {
					childList: true,
					characterData: true,
					subtree: true
				});
