@foreach( $variations as $variation)
    <tr @if(!empty($purchase_order_line)) data-purchase_order_id="{{$purchase_order_line->transaction_id}}" @endif @if(!empty($purchase_requisition_line)) data-purchase_requisition_id="{{$purchase_requisition_line->transaction_id}}" @endif>
        ...
        <td>
            ...
            <!-- PURCHASE TYPE UNDER PRODUCT NAME -->
            @if(!empty($purchase_types))
                <br>
                <select name="purchases[{{ $row_count }}][purchase_type_id]" class="form-control input-sm" style="margin-top: 11px;">
                    <option value="">Select Purchase Type</option>
                    @foreach($purchase_types as $id => $name)
        ...
        <td>
            ...
            @php
                ...
                $base_unit_multiplier = $base_unit_multiplier ?? 1;

                // Calculate purchase_quantity
                $purchase_quantity = $quantity_value / $base_unit_multiplier;
                ...
                // Calculate max_quantity with unit conversion
                ...
                if (!empty($purchase_order_line) && isset($remaining_quantities[$purchase_order_line->id])) {
                    $max_quantity = $remaining_quantities[$purchase_order_line->id]['display'];
                }
            ...
            <input type="text" 
                ...
                value="{{@format_quantity($purchase_quantity)}}"
                ...
                @if(!empty($max_quantity))
                    ...
                    @if(!empty($purchase_order_line) && isset($remaining_quantities[$purchase_order_line->id]))
                        data-max-qty-base="{{$remaining_quantities[$purchase_order_line->id]['base']}}"
                        data-current-multiplier="{{$remaining_quantities[$purchase_order_line->id]['multiplier']}}"
                    @endif
                @endif
            >

            @if(!empty($purchase_order_line) && isset($remaining_quantities[$purchase_order_line->id]) && $max_quantity > 0)
                <small class="max-qty-error" style="color: red; font-size: 12px; display: none;">
                    Max {{@format_quantity($max_quantity)}} allowed
                </small>
            @endif
            ...
            @if(!empty($sub_units))
                ...
                <select name="purchases[{{ $row_count }}][sub_unit_id]" class="form-control input-sm sub_unit">
                    @foreach($sub_units as $unit_id => $unit)
                        <option value="{{ $unit_id }}" data-multiplier="{{ $unit['multiplier'] }}" 
        ...
        <!-- Rest of the columns remain exactly the same as original -->
        <td>
            @php
