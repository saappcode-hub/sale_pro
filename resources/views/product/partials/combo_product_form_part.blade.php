...
<script type="text/javascript">
	$(document).ready(function(){
		...
		// Check if the type is combo_single
		const isComboSingle = $("#product_type").val() === "combo_single";
		...
		function get_product_entry_row(product_id, variation_id) {
			// If type is combo_single, ensure only one product can be added
			if (isComboSingle && $(".combo_product_table tbody tr").length > 0) {
				swal({
                    title: "Limit Reached",
                    text: "You can only add one product for combo single type.",
                    icon: "warning",
                    buttons: false,
                    timer: 3000
                });
			...
			if (product_id) {
				$.ajax({
					...
					success : function(result) {
						var newRow = $(result).appendTo(".combo_product_table tbody");
						update_combo_product_row_values(newRow);
						update_net_total_amount();
					}
				});
			}
