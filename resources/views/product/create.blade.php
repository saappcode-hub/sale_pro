...
<section class="content">
    ...
    <div class="row">
        ...
        <div class="col-sm-4">
            <div class="form-group">
                ...
                <div class="input-group">
                    {!! Form::select('purchase_unit', $units, !empty($duplicate_product->unit_id) ? $duplicate_product->unit_id : session('business.default_unit'), ['class' => 'form-control select2', 'required']); !!}
                    <span class="input-group-btn">
                        <button type="button" @if(!auth()->user()->can('unit.create')) disabled @endif class="btn btn-default bg-white btn-flat btn-modal" data-href="{{action([\App\Http\Controllers\UnitController::class, 'create'], ['quick_add' => true])}}" title="@lang('unit.add_unit')" data-container=".view_modal"><i class="fa fa-plus-circle text-primary fa-lg"></i></button>
                    </span>
                </div>
        ...
        <div class="col-sm-4">
            <div class="form-group">
                <br>
                <label>
                    {!! Form::checkbox('kind_product', 1, (!empty($duplicate_product) && $duplicate_product->kind_product == 1) ? true : false, ['class' => 'input-icheck', 'id' => 'kind_product_checkbox']) !!} <strong>@lang('Other Product')</strong>
                </label>
    ...
    @component('components.widget', ['class' => 'box-primary'])
    <div class="row">
        <!-- Product Type (Circle Box - Radio Buttons) -->
        <div class="col-sm-4">
            <div class="form-group">
                ...
                <div class="radio-circle-group">
                    <label class="radio-circle">
                        <input type="radio" name="product_type" value="1" checked> <!-- Default selection for "Normal" -->
                        <span class="radiomark-circle"></span> @lang('Normal')
                    </label>
                    <label class="radio-circle">
                        <input type="radio" name="product_type" value="2" id="wholesale_checkbox"> 
        ...
        <div class="col-sm-4">
            <div class="form-group">
                ...
                <!-- <div id="checkgropuID" style="color: red; font-weight: bold;">aassas</div> -->
                <div style="height: 5px;"></div>
                <div class="checkbox-group">
                    @foreach($selling_price_group_modify as $group)
                        <label class="checkbox">
                            <input type="checkbox" name="selling_price_group[]" value="{{ $group->id }}" required
                                {{ $group->is_default === 1 ? 'checked' : '' }}> <!-- Default selection for "Normal" -->
...
<script type="text/javascript">
   $(document).ready(function () {
    ...
    let $productTypeSelect = $('select[name="type"]');
    let $wholesaleCheckbox = $('#wholesale_checkbox');

    // Store "Variable" option in memory
    let $variableOption = $('<option value="variable">Variable</option>');

    $('input[name="product_type"]').on('change', function () {
        let selectedValue = $(this).val();

        if (selectedValue == "2") { // If Wholesale is selected
            $productTypeSelect.find('option[value="variable"]').remove();
        } else if (selectedValue == "1") { // If Normal is selected
            // Restore "Variable" before "Combo" if not already present
            if ($productTypeSelect.find('option[value="variable"]').length === 0) {
                $productTypeSelect.find('option[value="combo"]').before($variableOption);
            }
        }
    });
    
    // Fetch and update purchase units based on selected unit
    $('#unit_id').on('change', function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                let $purchaseUnit = $('#purchase_unit');
                $purchaseUnit.empty();

                $.each(data, function (key, value) {
                    $purchaseUnit.append(`<option value="${key}">${value}</option>`);
                });

                // Auto-select the initial unit
                $purchaseUnit.val(unit_id).trigger('change');
            },
    ...
    // Set default value for kind_product checkbox
    $('#kind_product_checkbox').prop('checked', false).on('change', function () {
        $('input[name="kind_product"]').val($(this).is(':checked') ? 1 : 0);
    });

    // Prevent accidental form exit
    __page_leave_confirmation('#product_add_form');

    // Enable barcode scanning for SKU input
    onScan.attachTo(document, {
        suffixKeyCodes: [13], // Enter key triggers scan
        reactToPaste: true,
