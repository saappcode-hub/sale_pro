<tr class="product_row">
                ...
                <select class="form-control lot_number" name="products[{{$row_index}}][lot_no_line_id]">
                    ...
                    @foreach($product->lot_numbers as $lot_number)
                        @php
                            ...
                            $max_qty_rule = $selected ? $lot_number->qty_available : $max_qty_rule;
                            $max_qty_msg = $selected ? __('lang_v1.quantity_error_msg_in_lot', ['qty' => $lot_number->qty_formated, 'unit' => $product->unit]) : $max_qty_msg;
    ...
    <td>
        @php
            ...
            $unit_name = $product->unit; // Default unit name
        @endphp
        @if(!empty($sub_units))
            @foreach($sub_units as $key => $value)
                @if(!empty($product->sub_unit_id) && $product->sub_unit_id == $key)
                    @php
                        // Safely access multiplier, default to 1 if not set
                        $multiplier = isset($value['multiplier']) ? $value['multiplier'] : 1;
                        $max_qty_rule = $multiplier > 0 ? $max_qty_rule / $multiplier : $max_qty_rule;
                        $unit_name = isset($value['name']) ? $value['name'] : $product->unit;
                        ...
                        if (isset($value['allow_decimal']) && $value['allow_decimal']) {
                            $allow_decimal = true;
                        }
                    @endphp
                @endif
            @endforeach
            @php
                $qty_ordered = $multiplier > 0 ? $product->quantity_ordered / $multiplier : $product->quantity_ordered;
            @endphp
        @endif
        ...
        @if(!empty($sub_units))
            ...
            <select name="products[{{$row_index}}][sub_unit_id]" class="form-control input-sm sub_unit">
                @foreach($sub_units as $key => $value)
                    <option value="{{$key}}" data-multiplier="{{isset($value['multiplier']) ? $value['multiplier'] : 1}}" data-unit_name="{{isset($value['name']) ? $value['name'] : $product->unit}}" data-allow_decimal="{{isset($value['allow_decimal']) ? $value['allow_decimal'] : 0}}" @if(!empty($product->sub_unit_id) && $product->sub_unit_id == $key) selected @endif>
                        {{isset($value['name']) ? $value['name'] : $product->unit}}
                    </option>
                @endforeach
            </select>
        @else 
