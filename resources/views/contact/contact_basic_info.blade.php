...
<!-- Area (Province, District, Commune) -->
@if(!empty($area))
    <strong><i class="fa fa-map-marker margin-r-5"></i>Area</strong>
    <p class="text-muted">
        @if(!empty($area->province_name)) {!! $area->province_name !!} @endif
        @if(!empty($area->district_name)) {!! $area->district_name !!} @endif
        @if(!empty($area->commune_name)) {!! $area->commune_name !!} @endif
    </p>
@endif
