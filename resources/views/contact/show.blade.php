        ...
        <div class="col-md-12">
            <div class="nav-tabs-custom">
                <ul class="nav nav-tabs nav-justified">
                    ...
                    <li class="@if(!empty($view_type) && $view_type == 'customer_contract') active @else '' @endif">
                        <a href="#customer_contract_tab" data-toggle="tab" aria-expanded="true">
                            <i class="fas fa-file-contract" aria-hidden="true"></i> Customer Contract
                        </a>
                    </li>

                    <!-- Shipping Address Tab -->
                    <li class="@if(!empty($view_type) && $view_type == 'shipping_address') active @else '' @endif">
                        <a href="#shipping_address_tab" data-toggle="tab" aria-expanded="true">
                            <i class="fas fa-truck" aria-hidden="true"></i> @lang('lang_v1.shipping_address')
                        </a>
                    </li>
                ...
                <div class="tab-content">
                    ...
                    <div class="tab-pane @if(!empty($view_type) && $view_type == 'shipping_address') active @else '' @endif" id="shipping_address_tab">
                        <input type="hidden" name="contact_id" id="contact_id_shipping" value="{{$contact->id}}">
                    ...
                    <!-- Customer Contract Tab Content -->
                    <div class="tab-pane @if(!empty($view_type) && $view_type == 'customer_contract') active @else '' @endif" id="customer_contract_tab">
                        <input type="hidden" name="contact_id" id="contact_id_contract" value="{{$contact->id}}">
                        <div class="customer_contract_body">
                            @include('contact.partials.customer_contract_table')
                        </div>
                    </div>
...
$(document).ready( function(){
    ...
    rp_log_table = $('#rp_log_table').DataTable({
        ...
        columns: [
            ...
            // --- FIX START: Add defaultContent ---
            { data: 'rp_earned', name: 'transactions.rp_earned', defaultContent: "0" },
            { data: 'rp_redeemed', name: 'transactions.rp_redeemed', defaultContent: "0" },
            // --- FIX END ---
        ]
    });
...
$('a[href="#customer_contract_tab"]').on('shown.bs.tab', function(e) {
    if (!$.fn.DataTable.isDataTable('#customer_contract_table')) {
        initializeCustomerContractDataTable();
    }
});
...
<script src="{{ asset('js/shipping_address.js?v=' . $asset_v) }}"></script>

<!-- customer_contract.js -->
<script src="{{ asset('js/customer_contract.js?v=' . $asset_v) }}"></script>
...
.set_default_checkbox:checked {
    background-color: #28a745; /* Green background */
    border-color: #28a745;
}

.set_default_checkbox:checked::after {
...
.map-link:hover {
    text-decoration: none; /* Remove underline on hover, consistent with typical link behavior */
}
.no-link {
    cursor: default; /* Prevents pointer cursor */
    color: #000; /* Black text for non-links, adjust if needed */
    text-decoration: none; /* Ensure no underline */
}
/* Force jQuery UI Autocomplete to be on top of Bootstrap Modals */
.ui-autocomplete {
