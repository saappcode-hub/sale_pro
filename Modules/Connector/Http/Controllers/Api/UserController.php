    ...
    public function index()
{
    ...
    public function loggedin()
    {
        ...
        $user['business_location'] = array_map(function($item){ return $item['id']; }, json_decode($user['business']['locations'],true));

         if(request()->dara == 1){}
         $filter = request()->only(['driver_app']);
         $footerText = DB::table('invoice_qrs')
                                 ->join('business_locations', 'business_locations.invoice_qrs_id', '=', 'invoice_qrs.id')
                                 ->where('invoice_qrs.business_id', $user->business_id)
                                 ->value('image');

            if (!empty($filter['driver_app']== 1)) {
                $appUpdates = DB::table('app_updates')->where('driver_app', $filter['driver_app'])->orderBy('id', 'desc')->get()->all();
            }else{

                $appUpdates = DB::table('app_updates')->where('driver_app', 0)->orderBy('id', 'desc')->get()->all();
            }
        $profileImage = DB::table('media')
                    ->where('media.business_id', $user->business_id)
                    ->where('media.uploaded_by', $user->id)
                    ->where('media.model_type', 'App\User')
                    ->select('file_name')
                    ->first(); 



         $defaultCurrency = DB::table('middle_currencies')
                                 ->where('currency_id', 21)
                                 ->where('store_id', $user->business_id)
                                 ->value('exchange_rate');

         $user['default_currency'] = $defaultCurrency ?? 4100;
         $user['business']['middle_currency']['exchange_rate'] = $user['business']['middle_currency']['myCurrency']['exchange_rate'] ?? 0;
         $user['footer_text'] = $footerText ?? '';
         $user['app_updates'] = $appUpdates ?? [];
         $user['profile_image'] = $profileImage ?? null;

         unset($user['business']['middle_currency']['myCurrency']);

        if (! $user->is_admin) {
            $user->all_permissions = $user->getAllPermissions()->pluck('name');
            $permittedLocations = auth()->user()->permitted_locations();
            $user['business_location'] = $permittedLocations != "all" ? $permittedLocations : $user['business_location'];
        }
