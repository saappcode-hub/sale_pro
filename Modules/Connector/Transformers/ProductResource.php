...
class ProductResource extends JsonResource
{
    ...
    { 
        ...
        $array['image_url'] = $this->imagePath($this->image, $this->business_id, $this->img_path_define);
    ...
    private function imagePath($img, $business_id, $path = 1){
        if($img != ''){
            $saleProPath = 'https://salepro.asia/uploads/img/';
            $s3Path = 'https://piik-data.sgp1.digitaloceanspaces.com/piik-data/salepro/public/image/';
    
            return $path == 1 ? $saleProPath.$img : $s3Path.$business_id . '/' . $img;
        }
        return;
    }
}
