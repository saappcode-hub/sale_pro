...
$(document).ready(function() {
    ...
    $(document).on('click', '.submit_product_form', function(e) {
        ...
        var is_valid_product_form = true;
        var variation_skus = [];
        var clickedButton = $(this); // Store clicked button reference
    
        // ✅ Set submit_type before validation
        var submit_type = clickedButton.val();
        $('#submit_type').val(submit_type);
        ...
        if (variation_skus.length > 0) {
            $.ajax({
                ...
                success: function (result) {
                    if (result.success == true) {
                        // ✅ Ensure `submit_type` is set again before submitting
                        $('#submit_type').val(submit_type);
            ...
            // ✅ Ensure `submit_type` is set before direct submission
            $('#submit_type').val(submit_type);
