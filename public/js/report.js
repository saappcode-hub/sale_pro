...
sell_payment_report = $('table#sell_payment_report_table').DataTable({
    processing: true,
    serverSide: true,
    aaSorting: [[3, 'desc']], // Keep sorting by paid_on date
    ajax: {
        url: '/reports/sell-payment-report',
        data: function(d) {
            ...
            // Check for URL parameters for paid_on date filtering
            ...
            if (paidOnDate) {
                // If we have paid_on parameter, use it for filtering
                d.paid_on = paidOnDate;
                console.log('Using paid_on filter:', paidOnDate);
            } else if ($('input#spr_date_filter').val()) {
                // Otherwise use the date range picker
                var dateRangePicker = $('input#spr_date_filter').data('daterangepicker');
                if (dateRangePicker) {
                    start = dateRangePicker.startDate.format('YYYY-MM-DD');
                    end = dateRangePicker.endDate.format('YYYY-MM-DD');
                    d.start_date = start;
                    d.end_date = end;
                    console.log('Using date range filter:', start, 'to', end);
                }
            }
        },
    },
    columns: [
        ...
        { data: 'payment_ref_no', name: 'payment_ref_no' },
        { data: 'invoice_create_date', name: 't.transaction_date' },
        { data: 'paid_on', name: 'paid_on' },
        { data: 'invoice_no', name: 't.invoice_no' },
        { data: 'customer', orderable: false, searchable: false },
        { data: 'customer_group', name: 'customer_group', searchable: false},
        { data: 'method', name: 'method' },
        { data: 'total_amount', name: 'total_amount', searchable: false }, // NEW: Total Amount column
        { data: 'amount', name: 'transaction_payments.amount' }, // Amount Paid
        { data: 'amount_percentage', name: 'amount_percentage', orderable: false, searchable: false },
        { data: 'action', orderable: false, searchable: false },
    ],
    fnDrawCallback: function(oSettings) {
    // Calculate totals for all three amount columns
    var total_invoice_amount = 0;
    var total_amount = sum_table_col($('#sell_payment_report_table'), 'paid-amount');
    var total_amount_percentage = sum_table_col($('#sell_payment_report_table'), 'amount-percentage');
    
    // For Total Amount column, only sum unique invoice amounts (avoid duplicates)
    var seen_invoices = {};
    var data = this.api().rows({page: 'current'}).data();
    
    for (var i = 0; i < data.length; i++) {
        var row = data[i];
        var invoice_no = $(row.invoice_no).text() || row.invoice_no; // Extract text from HTML
        
        // Only add to total if we haven't seen this invoice before
        if (invoice_no && !seen_invoices[invoice_no]) {
            seen_invoices[invoice_no] = true;
            
            // Extract total amount value
            if (row.total_amount && typeof row.total_amount === 'string') {
                var totalAmountMatch = row.total_amount.match(/data-orig-value="([^"]*)"/);
                if (totalAmountMatch && totalAmountMatch[1]) {
                    total_invoice_amount += parseFloat(totalAmountMatch[1]) || 0;
                }
            }
        }
    }
    ...
    $('#footer_total_invoice_amount').text(__currency_trans_from_en(total_invoice_amount, true));
    $('#footer_total_amount').text(total_amount);
    $('#footer_total_amount_percentage').text(total_amount_percentage);
...
if ($('#spr_date_filter').length == 1) {
    $('#spr_date_filter').daterangepicker(dateRangeSettings, function(start, end) {
        var format = moment_date_format || 'MM/DD/YYYY';
        $('#spr_date_filter').val(
            start.format(format) + ' ~ ' + end.format(format)
        );
        
        // Clear URL parameters when user manually changes date range
        if (window.location.search.includes('paid_on')) {
            var url = new URL(window.location);
            url.searchParams.delete('paid_on');
            url.searchParams.delete('sell_list_filter_location_id');
    ...
    $('#spr_date_filter').on('cancel.daterangepicker', function(ev, picker) {
        ...
        // Clear URL parameters when user clears date range
        if (window.location.search.includes('paid_on')) {
            var url = new URL(window.location);
            url.searchParams.delete('paid_on');
            url.searchParams.delete('sell_list_filter_location_id');
