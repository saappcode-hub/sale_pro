$(document).ready(function() {
    ...
    var current_qty = __read_number(quantity_input, true);
    var max_qty_base = parseFloat(quantity_input.attr('data-max-qty-base')) || 0;
    var current_multiplier = parseFloat(quantity_input.attr('data-current-multiplier')) || 1;
    var max_error_msg = row.find('.max-qty-error');
    
    // Check if this row has purchase order validation
    if (max_qty_base > 0) {
        // Get current unit multiplier
        var selected_multiplier = 1;
        var sub_unit_select = row.find('select.sub_unit');
        if (sub_unit_select.length > 0) {
            selected_multiplier = parseFloat(sub_unit_select.find(':selected').data('multiplier')) || 1;
        } else {
            selected_multiplier = current_multiplier;
        }
        
        // Convert current quantity to base units
        var current_qty_base = current_qty * selected_multiplier;
        
        // Show/hide error message based on validation
        if (current_qty_base > max_qty_base) {
            max_error_msg.show();
        } else {
            max_error_msg.hide();
        }
    ...
    var sub_total_before_tax = current_qty * purchase_before_tax;
    var sub_total_after_tax = current_qty * purchase_after_tax;
...
$(document).on('click', 'button#submit_purchase_form', function(e) {
    ...
    // ADD THIS NEW VALIDATION BLOCK FOR PURCHASE TYPE
    var purchase_type_required = false;
    var hasEmptyPurchaseType = false;

    // Check if purchase type feature is enabled
    $('table#purchase_entry_table tbody tr').each(function() {
        var row = $(this);
        var purchase_type_select = row.find('select[name*="[purchase_type_id]"]');
        
        // If purchase type dropdown exists in this row, it's required
        if (purchase_type_select.length > 0) {
            purchase_type_required = true;
            var selected_value = purchase_type_select.val();
            
            // Check if no purchase type is selected
            if (!selected_value || selected_value === '') {
                hasEmptyPurchaseType = true;
                purchase_type_select.css('border', '2px solid red');
                return false; // break the loop
            } else {
                purchase_type_select.css('border', '');
            }
        }
    });

    // If purchase type is required and not selected, show error
    if (purchase_type_required && hasEmptyPurchaseType) {
        toastr.error('Purchase Type is required for all products. Please select a purchase type.');
        return false;
    }
    // END OF PURCHASE TYPE VALIDATION

    // ADD THIS VALIDATION BLOCK FOR UNIT CONVERSION
    var hasQuantityError = false;
    $('table#purchase_entry_table tbody tr').each(function() {
        var row = $(this);
        var quantity_input = row.find('input.purchase_quantity');
        var current_qty = __read_number(quantity_input, true);
        var max_qty_base = parseFloat(quantity_input.attr('data-max-qty-base')) || 0;
        var current_multiplier = parseFloat(quantity_input.attr('data-current-multiplier')) || 1;
        var max_error_msg = row.find('.max-qty-error');
        
        // Check if this row has purchase order validation (has data-max-qty-base)
        if (max_qty_base > 0) {
            // Get current unit multiplier
            var selected_multiplier = 1;
            var sub_unit_select = row.find('select.sub_unit');
            ...
            // Check if current quantity exceeds remaining quantity in base units
            if (current_qty_base > max_qty_base) {
                hasQuantityError = true;
                quantity_input.focus();
                max_error_msg.show(); // Show the error message
                var max_allowed_display = max_qty_base / selected_multiplier;
                toastr.error('Quantity ' + current_qty + ' exceeds maximum allowed ' + max_allowed_display.toFixed(2));
                return false; // Stop validation on first error
            } else {
                max_error_msg.hide(); // Hide the error message if valid
            }
        }
    });
            ...
            ref_no: {
                remote: {
                    ...
                    data: {
                        ...
                        original_ref_no: function() {
                            return $('#original_ref_no').val() || '';
                        },
                    },
                },
            },
