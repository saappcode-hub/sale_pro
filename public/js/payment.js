...
function set_default_payment_account() {
    ...
    if (!_.isUndefined($('#transaction_payment_add_form #default_payment_accounts').val())) {
        // FIXED: Use safe JSON parse instead of direct JSON.parse
        var defaultAccountsVal = $('#transaction_payment_add_form #default_payment_accounts').val();
        default_accounts = safeJsonParse(defaultAccountsVal, {});
    }
...
$(document).on('submit', 'form#transaction_payment_add_form', function(e){
    ...
    // FIXED: Use safe JSON parse instead of direct JSON.parse
    var denominationVal = $('#transaction_payment_add_form .enable_cash_denomination_for_payment_methods').val();
    var denomination_for_payment_types = safeJsonParse(denominationVal, []);
    
    if (Array.isArray(denomination_for_payment_types) && denomination_for_payment_types.includes(payment_type) && $('#transaction_payment_add_form .is_strict').length && $('#transaction_payment_add_form .is_strict').val() === '1' ) {
